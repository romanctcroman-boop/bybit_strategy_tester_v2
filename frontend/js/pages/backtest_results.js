...
function updateTVTradesListTab(trades, config) {
  ...
  if (countEl) {
    const closedCount = trades.filter((t) => !t.is_open).length;
    const openCount = trades.length - closedCount;
    countEl.textContent =
      openCount > 0 ? `${closedCount} (${openCount} открытых)` : String(trades.length);
  }
                        ...
                        <div class="result-content">
                            ...
                            <div class="result-row">
                                ...
                                <span class="result-pnl-value ${isProfitable ? 'text-success' : 'text-danger'}">
                                    ...
                                    ${isProfitable ? '+' : ''}${(r.metrics?.net_profit_pct ?? (r.metrics?.total_return || 0) * 100).toFixed(2)}%
...
function updateMetrics(metrics) {
  ...
  // Core metrics
  // net_profit_pct is already in % (backend-authoritative); total_return is a 0-1 fraction fallback
  setMetric('metricReturn', metrics.net_profit_pct ?? (metrics.total_return || 0) * 100, 'percent', 0);
...
function updateCharts(backtest) {
  ...
  if (
    ...
    try {
      ...
      // Filter out open (unrealised) trades — only count closed trades
      const closedTrades = backtest.trades.filter((t) => !t.is_open);
...
const HEATMAP_METRIC_GROUPS = [

  {
    ...
    metrics: [
      ...
      { key: 'net_profit_pct', label: 'Total Return', format: 'pct', goodWhen: 'high' },

      { key: 'cagr', label: 'CAGR %', format: 'pct', goodWhen: 'high' },

      { key: 'avg_trade', label: 'Avg Trade', format: 'currency', goodWhen: 'high' }
...

  {
    ...
    metrics: [
      ...
      { key: 'max_drawdown', label: 'Max DD %', format: 'pct', goodWhen: 'low', negate: true },

      { key: 'avg_drawdown', label: 'Avg DD %', format: 'pct', goodWhen: 'low', negate: true },
...
function renderMetricsHeatmap(metrics) {
  ...
  const groups = HEATMAP_METRIC_GROUPS.map((group) => {
    ...
    const cells = group.metrics.map((m) => {
      ...
      const rawValue = raw !== undefined && raw !== null ? parseFloat(raw) : null;

      const value = rawValue !== null && m.multiply ? rawValue * m.multiply : rawValue;

      // negate: backend stores drawdowns as positive (9.9), display as negative (-9.9%)
      const displayValue = value !== null && m.negate ? -Math.abs(value) : value;

      const colorClass = getHeatmapColor(m.key, displayValue, m.goodWhen);

      const formatted = formatHeatmapValue(displayValue, m.format);
...
function renderWalkForwardViz(wfData) {
  ...
  windowsContainer.innerHTML = `
    ...
    <div class="mb-2 d-flex gap-3 text-secondary small">
      ...
      <span>Совокупный OOS доход: <strong>${((wfData.combined_oos_return ?? 0)).toFixed(1)}%</strong></span>
