...
function setupEventListeners() {
    ...
    document.querySelectorAll('.bottom-tab').forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            tab.classList.add('active');
            // Switch bottom panel content based on tab
                const tabText = tab.textContent.trim();
                const tbody = document.getElementById('positionsTableBody');
                if (!tbody) return;
                if (tabText.startsWith('Positions')) {
                    tbody.innerHTML = positionsData.map(p => '<tr><td>' + p.symbol + '</td><td class="' + (p.side==='BUY'?'text-success':'text-danger') + '">' + p.side + '</td><td>' + p.size + '</td><td>' + p.entryPrice + '</td><td>' + p.markPrice + '</td><td>' + p.liqPrice + '</td><td>' + p.margin + '</td><td class="' + (p.pnl>=0?'text-success':'text-danger') + '">' + (p.pnl>=0?'+':'') + p.pnl + ' (' + (p.pnlPercent>=0?'+':'') + p.pnlPercent + '%)</td></tr>').join('');
                } else if (tabText.startsWith('Open Orders')) {
                    tbody.innerHTML = openOrdersData.map(o => '<tr><td>' + o.symbol + '</td><td class="' + (o.side==='BUY'?'text-success':'text-danger') + '">' + o.side + '</td><td>' + o.type + '</td><td>' + o.qty + '</td><td>' + o.price + '</td><td>-</td><td>-</td><td>' + o.status + '</td></tr>').join('');
                } else {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (tickers.length > 0) {
                    ...
                    renderWatchlist();
                    // Update mark price display (BUG 4 fix)
                    const markSpan = document.querySelector('span.text-muted');
                    if (markSpan && markSpan.textContent.includes('Mark:')) {
                        const markData = tickers.find(s => s.symbol === currentSymbol);
                        if (markData) {
                            markSpan.textContent = 'Mark: ' + Number(markData.price).toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2});
                        }
                    }
