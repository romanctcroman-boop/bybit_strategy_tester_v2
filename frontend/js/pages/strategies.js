...
function init() {
    ...
    } catch (e) { console.error('[strategies.js] setUICallbacks FAILED:', e); }
    ...
    } catch (e) { console.error('[strategies.js] expose loadStrategies FAILED:', e); }

    try { loadStrategies(); console.log('[strategies.js] loadStrategies started'); }
    catch (e) { console.error('[strategies.js] loadStrategies FAILED:', e); }

    try { loadStrategyTypes(); console.log('[strategies.js] loadStrategyTypes started'); }
    catch (e) { console.error('[strategies.js] loadStrategyTypes FAILED:', e); }

    try { loadTopSymbols(); console.log('[strategies.js] loadTopSymbols started'); }
    catch (e) { console.error('[strategies.js] loadTopSymbols FAILED:', e); }

    try { setupFilters(); console.log('[strategies.js] setupFilters done'); }
    catch (e) { console.error('[strategies.js] setupFilters FAILED:', e); }

    try { setDefaultDates(); console.log('[strategies.js] setDefaultDates done'); }
    catch (e) { console.error('[strategies.js] setDefaultDates FAILED:', e); }

    try { setupEventListeners(); console.log('[strategies.js] setupEventListeners done'); }
    catch (e) { console.error('[strategies.js] setupEventListeners FAILED:', e); }

    try { initLeverageSliderScroll(); console.log('[strategies.js] initLeverageSliderScroll done'); }
    catch (e) { console.error('[strategies.js] initLeverageSliderScroll FAILED:', e); }
...
checkSymbolData = debounce(async function () {
    ...
    if (!symbol || !timeframe) {
        console.log('[checkSymbolData] Missing symbol or timeframe, hiding status row');
...
function updateParametersUI(existingParams = {}) {
    ...
    // Container was removed - parameters are now managed via optimizer
    if (!container) return;
...
function getParametersFromUI() {
    // First try to read from UI inputs (if they exist)
    const params = {};
    ...
    // If no UI params found, return default params for the strategy type
...
function updateTicksPerBar() {
    const subticks = parseInt(document.getElementById('intrabarSubticks')?.value || 1);
...
function getBarMagnifierConfig() {
    const useBarMagnifier = document.getElementById('useBarMagnifier')?.checked ?? true;  // ALWAYS true by default for accurate intrabar simulation
    const ohlcPath = document.getElementById('intrabarOhlcPath')?.value || 'O-HL-heuristic';
    const subticks = parseInt(document.getElementById('intrabarSubticks')?.value || 0);
