...
import { Box, Button, Container, Divider, IconButton, Stack, TextField, Tooltip, Typography } from '@mui/material';
...
const ActiveBotsPage: React.FC = () => {
  ...
  const stats = React.useMemo(() => {
    ...
    const avgEntry = totalQty > 0 ? items.reduce((s, it) => s + (it.entry * (it.qty ?? 0)), 0) / totalQty : 0;
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 2 }}>
      ...
      {/* Stats panel */}
      <Box sx={{ mb: 2, p: 2, bgcolor: '#fff', border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={3} divider={<Divider orientation="vertical" flexItem />}>
          <Stack>
            <Typography variant="caption" sx={{ color: 'text.secondary' }}>ВСЕГО СДЕЛОК</Typography>
            <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>{stats.count} <Typography component="span" variant="body2" sx={{ color: 'text.secondary', ml: 0.5 }}>({stats.countLong} L / {stats.countShort} S)</Typography></Typography>
          </Stack>
          <Stack>
            <Typography variant="caption" sx={{ color: 'text.secondary' }}>ALLOCIROVANO</Typography>
            <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>{stats.allocated.toFixed(2)} USDT</Typography>
          </Stack>
          <Stack>
            <Typography variant="caption" sx={{ color: 'text.secondary' }}>AVG ENTRY (w)</Typography>
            <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>{stats.avgEntry.toFixed(4)}</Typography>
          </Stack>
          <Stack>
            <Typography variant="caption" sx={{ color: 'text.secondary' }}>TOTAL P&L</Typography>
            <Typography variant="subtitle1" sx={{ fontWeight: 700, color: stats.pnlUsd >= 0 ? 'success.main' : 'error.main' }}>
              {stats.pnlUsd >= 0 ? '+' : '−'}{Math.abs(stats.pnlUsd).toFixed(2)} USDT <Typography component="span" variant="body2" sx={{ ml: 0.5, color: 'text.secondary' }}>({(stats.pnlPct >= 0 ? '+' : '−') + Math.abs(stats.pnlPct).toFixed(2)}%)</Typography>
            </Typography>
          </Stack>
        </Stack>
      </Box>
