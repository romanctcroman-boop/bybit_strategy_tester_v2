...
import { Container, Typography, Paper, Stack, TextField, MenuItem, Button, ButtonGroup } from '@mui/material';
...
const TradesTable: React.FC<{ trades: Trade[] }> = ({ trades }) => {
  const cols: GridColDef[] = [
    ...
    { field: 'entry_time', headerName: 'Entry', width: 170, valueGetter: (p: any) => new Date(p.row.entry_time).toLocaleString() },
    ...
    { field: 'drawdown', headerName: 'DD', width: 100, valueGetter: (p: any) => p.row.drawdown ?? '' },
    { field: 'bars_held', headerName: 'Bars', width: 90, valueGetter: (p: any) => p.row.bars_held ?? '' },
  ];
...
const BacktestDetailPage: React.FC = () => {
  ...
  const { fetchTrades, setTradesPage, setTradeFilters, trades, tradesLimit, tradesOffset, tradesTotal, tradeSide } = useBacktestsStore();
    ...
    (async () => {
      try {
        ...
        if (t) {
          ...
          const sorted = [...t].sort((a, b) => new Date(a.entry_time).getTime() - new Date(b.entry_time).getTime());
  ...
  const applyTradeFilter = async (side: 'buy' | 'sell' | '') => {
    ...
    const t = await fetchTrades(backtestId, { side: side || undefined, limit: pageSize, offset: 0 });
    if (t) {
      const sorted = [...t].sort((a, b) => new Date(a.entry_time).getTime() - new Date(b.entry_time).getTime());
      let cum = backtest?.initial_capital ?? 0;
      let peak = cum;
  ...
  const exportCsv = () => {
    ...
    const lines = [headers.join(',')].concat(rows.map(r => [r.id, r.backtest_id, new Date(r.entry_time).toISOString(), r.exit_time ? new Date(r.exit_time).toISOString() : '', r.side, r.price, r.qty, r.pnl ?? ''].join(',')));
  ...
  }, [tvSymbol, tvInterval, useTradingView]);
    ...
    <Container>
      ...
      {tab === 0 && backtest && (
        <Paper sx={{ mt: 2, p: 2 }}>
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid item xs={12} md={6} lg={4}><Chip label={`Strategy: ${backtest.strategy_id}`} /></Grid>
            <Grid item xs={12} md={6} lg={4}><Chip label={`Symbol: ${backtest.symbol}`} /></Grid>
            <Grid item xs={12} md={6} lg={4}><Chip label={`Timeframe: ${backtest.timeframe}`} /></Grid>
            <Grid item xs={12} md={6} lg={6}><Chip label={`Period: ${backtest.start_date} → ${backtest.end_date}`} /></Grid>
            <Grid item xs={12} md={6} lg={3}><Chip label={`Status: ${backtest.status}`} /></Grid>
            {backtest.final_capital != null && (
              <Grid item xs={12} md={6} lg={3}><Chip color="primary" label={`Final Capital: ${backtest.final_capital}`} /></Grid>
            )}
          </Grid>
          {backtest.metrics && (
            <div style={{ marginTop: 12 }}>
              <Typography variant="subtitle1">Metrics (raw)</Typography>
              <pre style={{ margin: 0, whiteSpace: 'pre-wrap' }}>{JSON.stringify(backtest.metrics, null, 2)}</pre>
            </div>
          )}
        </Paper>
      )}
        ...
        <>
          <Paper sx={{ mt: 2, p: 2 }}>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={series}>
                ...
                <XAxis dataKey="time" tickFormatter={(v: any) => new Date(v).toLocaleDateString()} />
                <YAxis />
                <Tooltip />
          ...
          <Paper sx={{ mt: 2, p: 2 }}>
            ...
            <ResponsiveContainer width="100%" height={160}>
              <AreaChart data={ddSeries}>
                ...
                <Area type="monotone" dataKey="drawdown" stroke="#ef5350" fill="#ef5350" fillOpacity={0.3} />
              </AreaChart>
            </ResponsiveContainer>
          </Paper>
          ...
          <Paper sx={{ mt: 2, p: 2 }}>
            ...
            <Stack direction="row" spacing={2} alignItems="center" sx={{ mb: 2 }}>
              ...
              {!useTradingView && (
                <>
                  <FormControlLabel control={<Checkbox checked={showSMA20} onChange={(_, v) => setShowSMA20(v)} />} label="SMA 20" />
                  <FormControlLabel control={<Checkbox checked={showSMA50} onChange={(_, v) => setShowSMA50(v)} />} label="SMA 50" />
                </>
              )}
            </Stack>
            {useTradingView ? (
              <>
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
                  <TextField label="Symbol" size="small" value={tvSymbol} onChange={(e) => setTvSymbol(e.target.value)} />
                  <TextField label="Interval" size="small" select value={tvInterval} onChange={(e) => setTvInterval(e.target.value)}>
                    {['1','5','15','60','240','D','W'].map((v) => (
                      <MenuItem key={v} value={v}>{v}</MenuItem>
                    ))}
                  </TextField>
                  <TextField label="Theme" size="small" select value={tvTheme} onChange={(e) => setTvTheme(e.target.value as any)}>
                    {['light','dark'].map((v) => (
                      <MenuItem key={v} value={v}>{v}</MenuItem>
                    ))}
                  </TextField>
                  <FormControlLabel control={<Checkbox checked={useMACD} onChange={(_, v) => setUseMACD(v)} />} label="MACD" />
                  <FormControlLabel control={<Checkbox checked={useRSI} onChange={(_, v) => setUseRSI(v)} />} label="RSI" />
                </div>
                <TradingViewWidget symbol={`BYBIT:${tvSymbol}`} interval={tvInterval} theme={tvTheme} studies={[
                  ...(useMACD ? ['MACD@tv-basicstudies'] : []),
                  ...(useRSI ? ['RSI@tv-basicstudies'] : []),
                ]} />
              </>
            ) : (
              <React.Suspense fallback={<div style={{ height: 480, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Loading chart...</div>}>
                <TradingViewChart
                  candles={candles}
                  markers={trades.map((t) => ({ time: Math.floor(new Date(t.entry_time).getTime() / 1000), side: t.side as any, price: t.price }))}
                  showSMA20={showSMA20}
                  showSMA50={showSMA50}
                />
              </React.Suspense>
      ...
      {tab === 2 && (
        <Paper sx={{ mt: 2, p: 2 }}>
          ...
          <Typography variant="body2" color="text.secondary">More analytics coming soon.</Typography>
        </Paper>
      )}
        ...
        <Paper sx={{ mt: 2, p: 2 }}>
          <Stack direction="row" justifyContent="space-between" alignItems="center">
            ...
            <Stack direction="row" spacing={2} alignItems="center">
              <TextField label="Side" size="small" select value={tradeSide || ''} onChange={(e) => applyTradeFilter((e.target.value as any) || '')} sx={{ width: 140 }}>
                <MenuItem value="">All</MenuItem>
                <MenuItem value="buy">Buy</MenuItem>
                <MenuItem value="sell">Sell</MenuItem>
              </TextField>
              <TextField label="Page size" size="small" type="number" value={pageSize} onChange={(e) => setPageSize(Math.max(10, Math.min(1000, Number(e.target.value) || 50)))} sx={{ width: 120 }} />
              <Button variant="outlined" onClick={() => changePage(1)}>Apply</Button>
          ...
          <Stack direction="row" spacing={2} alignItems="center" justifyContent="flex-end" sx={{ mt: 2 }}>
            ...
            <div>Page {page} / {Math.max(1, Math.ceil((tradesTotal || 0) / pageSize))}</div>
            <Button disabled={page >= Math.ceil((tradesTotal || 0) / pageSize)} onClick={() => changePage(page + 1)}>Next</Button>
          </Stack>
        </Paper>
      )}
    </Container>
