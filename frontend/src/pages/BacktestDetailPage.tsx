...
const formatSignedValueWithUnit = (
  value: unknown,
  unit: ValueUnit,
  digits?: number,
  fallback = '—'
...
const formatQuantity = (value: unknown, digits = 2): string => {
  const numeric = toFiniteNumber(value);
    ...
    },
    {
      ...
      secondary:
        wins != null && losses != null
          ? `${formatValueWithUnit(wins, 'plain', 0)} побед / ${formatValueWithUnit(losses, 'plain', 0)} поражений`
          : undefined,
    },
    {
      title: 'Win Rate',
...
const AnalysisTab: React.FC<{ results: BacktestResults }> = ({ results }) => {
  ...
  const fields: Array<{ key: keyof SideStats; label: string; unit: ValueUnit; digits?: number }> = [
    ...
    { key: 'open_trades', label: 'Открытые сделки', unit: 'plain' },
    { key: 'wins', label: 'Победы', unit: 'plain' },
    { key: 'losses', label: 'Поражения', unit: 'plain' },
    ...
    { key: 'avg_bars_win', label: 'Баров в победе', unit: 'plain' },
    { key: 'avg_bars_loss', label: 'Баров в поражении', unit: 'plain' },
  ];
