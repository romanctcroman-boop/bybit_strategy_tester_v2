...
import AdminDbStatusCard from '../components/AdminDbStatusCard';
...
const AdminBackfillPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (runsTimerRef.current) window.clearInterval(runsTimerRef.current);
  ...
  const trigger = async () => {
    ...
    try {
      ...
      const payload = await adminBackfill(
        { symbol, interval, lookback_minutes: lookback, mode },
        auth
      );
      setResult(payload);
      ...
      if (payload?.task_id) {
        ...
        timerRef.current = window.setInterval(async () => {
          try {
            ...
            if (payload.run_id) {
              try {
                setLiveRun(await adminGetRun(payload.run_id, auth));
              } catch {}
            }
            if (s?.ready) {
              if (timerRef.current) window.clearInterval(timerRef.current);
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ marginTop: 12 }}>
        ...
        <button style={{ marginLeft: 8 }} onClick={doResetProgress}>
          Reset Progress
        </button>
        <button
          style={{ marginLeft: 8 }}
          onClick={() => adminGetProgress(symbol, interval, { user, pass }).then(setProgress)}
        >
          Refresh Progress
        </button>
      </div>
      ...
      {(result?.eta_sec !== undefined || status?.result?.eta_sec !== undefined) && (
        <div style={{ marginTop: 16 }}>
          ...
          <div>
            Est pages left: {result?.est_pages_left ?? status?.result?.est_pages_left ?? 'n/a'}
          </div>
        </div>
      )}
      {progress && (
        ...
        <table style={{ borderCollapse: 'collapse', width: '100%', maxWidth: 900 }}>
          <thead>
            <tr>
              ...
              <th style={{ textAlign: 'left', borderBottom: '1px solid #ccc', padding: 4 }}>
                Symbol
              </th>
              <th style={{ textAlign: 'left', borderBottom: '1px solid #ccc', padding: 4 }}>
                Interval
              ...
              <th style={{ textAlign: 'right', borderBottom: '1px solid #ccc', padding: 4 }}>
                Upserts
              </th>
              <th style={{ textAlign: 'right', borderBottom: '1px solid #ccc', padding: 4 }}>
                Pages
                ...
                <td style={{ padding: 4 }}>
                  <a
                    ...
                    onClick={async (e) => {
                      ...
                      try {
                        const auth = { user, pass };
                        setLiveRun(await adminGetRun(r.id, auth));
                      } catch {}
                    }}
                  >
                    {r.id}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '180px 1fr', gap: 8, maxWidth: 640 }}>
        ...
        <input
          ...
          value={archiveBeforeIso}
          onChange={(e) => setArchiveBeforeIso(e.target.value)}
        />
        ...
        <select
          value={archiveIntervalForPartition}
          onChange={(e) => setArchiveIntervalForPartition(e.target.value)}
        >
          {['1', '3', '5', '15', '30', '60', '240', 'D', 'W'].map((i) => (
            <option key={i} value={i}>
              {i}
        ...
        <input
          ...
          onChange={(e) => setArchiveBatchSize(Number(e.target.value || 0))}
        />
        <label>Mode</label>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
        <button
          onClick={async () => {
            try {
              const payload = await adminRestore(
                { input_dir: restoreDir, mode: restoreMode },
                { user, pass }
              );
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        <input
          style={{ width: 360 }}
          value={archivesDir}
          onChange={(e) => setArchivesDir(e.target.value)}
