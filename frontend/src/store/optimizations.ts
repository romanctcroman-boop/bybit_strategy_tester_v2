...
import { optimizationsPersistConfig } from './persistConfig';
...
export const useOptimizationsStore = create<OptimizationsState>()(
  persist(
    (set, get) => ({
      ...
      selectedId: null,
      setFilters: (f: { status?: string; strategyId?: number }) =>
        set({ status: f.status, strategyId: f.strategyId }),
