...
interface TradeMarker { time: number; side: 'buy' | 'sell'; price: number }
interface Props { candles: Candle[]; markers?: TradeMarker[]; showSMA20?: boolean; showSMA50?: boolean }

const TradingViewChart: React.FC<Props> = ({ candles, markers = [], showSMA20 = true, showSMA50 = false }) => {
  ...
  const addCandlesCompat = (chart: any, options?: any) => {
    ...
    try {
      if (typeof chart?.addSeries === 'function') return chart.addSeries({ type: 'Candlestick', ...(options || {}) });
    } catch (e) {}
  ...
  const addLineCompat = (chart: any, options?: any) => {
    ...
    try {
      if (typeof chart?.addSeries === 'function') return chart.addSeries({ type: 'Line', ...(options || {}) });
    } catch (e) {}
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error('Failed to initialize TradingViewChart:', e);
      return undefined;
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (showSMA20) {
        if (!sma20Ref.current) sma20Ref.current = addLineCompat(chartRef.current, { color: '#0288d1', lineWidth: 1 });
        sma20Ref.current.setData(toSMA(20) as any);
      } else if (sma20Ref.current) {
        sma20Ref.current.setData([]);
      }
      if (showSMA50) {
        if (!sma50Ref.current) sma50Ref.current = addLineCompat(chartRef.current, { color: '#7b1fa2', lineWidth: 1 });
  ...
  useEffect(() => {
    ...
    try {
      if (markers.length) {
        seriesRef.current.setMarkers(markers.map((m: any) => ({
          time: Math.floor(m.time),
          position: m.side === 'buy' ? 'belowBar' : 'aboveBar',
          color: m.side === 'buy' ? '#2e7d32' : '#c62828',
          shape: m.side === 'buy' ? 'arrowUp' : 'arrowDown',
