...
def find_divergence():
    ...
    print("=" * 80)
    print(
        f"{'#':>3} | {'TV Time':>19} | {'TV Dir':>6} | {'TV Price':>10} | {'Our Time':>19} | {'Our Dir':>6} | {'Our Price':>10} | {'Match':>6}"
    )
    print("-" * 120)
    ...
    for i in range(min(30, len(tv_trades))):
        ...
        if i < len(our_signals):
            ...
            # Match criteria
            is_match = (
                time_diff <= tolerance_minutes
                and price_diff < 100  # $100 tolerance
                and dir_match
            )
            ...
            print(
                f"{i + 1:>3} | {str(tv['datetime_utc'])[:19]:>19} | {tv['direction']:>6} | ${tv['price']:>9.2f} | {str(our['datetime'])[:19]:>19} | {our['direction']:>6} | ${our['price']:>9.2f} | {match_str}"
            )
        else:
            print(
                f"{i + 1:>3} | {str(tv['datetime_utc'])[:19]:>19} | {tv['direction']:>6} | ${tv['price']:>9.2f} | {'N/A':>19} | {'N/A':>6} | {'N/A':>10} | ❌"
            )
    ...
    for i in range(min(10, len(tv_trades))):
        ...
        if price_diffs[closest_idx] < 1:  # Exact match
            ...
            print(
                f"TV #{i + 1} (${tv['price']:.2f}) → Our #{our_num} (${closest['price']:.2f}) | Offset: {our_num - (i + 1):+d}"
            )
