...
def main():
    ...
    if not preferred and instruments:
        preferred = (
            instruments[0].get("symbol") if isinstance(instruments[0], dict) else instruments[0]
        )
    ...
    if not ok:
        print(
            "No kline data found for preferred/popular symbols. You can set BYBIT_SYMBOL env to try a specific contract."
        )
