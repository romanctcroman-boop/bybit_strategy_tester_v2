"""Test baseline (no 5m intra-bar) to confirm ~150 trades still works."""
...
async def run():
    ...
    try:
        ...
        print(f"BTC 30m: {len(btc_candles)} bars (with warmup)")
    ...
    print(f"Trades: {len(trd)} ({len(longs)}L + {len(shorts)}S)")
    print("\nFirst 10 trades:")
    for t in trd[:10]:
        print(f"  {t.direction:5s}  {str(t.entry_time)[:16]}  ep={t.entry_price:.2f}")
