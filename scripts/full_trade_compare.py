...
The matcher pairs trades by side and nearest entry_time and produces detailed
per-trade diffs (time, price, size, pnl, fees, mfe/mae).
"""
...
def to_dataframe(data: np.ndarray) -> pd.DataFrame:
    ...
    def trade_to_dict(t: Any) -> Dict[str, Any]:
        # Try pydantic model_dump / dict, fallback to __dict__
        try:
    ...
    def pairwise_match(fb_trades: List[Dict[str, Any]], vb_trades: List[Dict[str, Any]], max_time_diff_seconds: int = 86400) -> Dict[str, Any]:
        """Match fallback trades to vectorbt trades by side + nearest entry_time.

        Returns a dict with matches list and unmatched lists for both sides.
        """
        ...
        for i, ft in enumerate(fb_trades):
            ...
            if best_j is not None and best_dt is not None and best_dt <= max_time_diff_seconds:
                ...
                match = {
                    ...
                    "size": {"fallback": size_a, "vectorbt": size_b, "diff": None if size_a is None or size_b is None else size_a - size_b},
                    "pnl": {"fallback": pnl_a, "vectorbt": pnl_b, "diff": None if pnl_a is None or pnl_b is None else pnl_a - pnl_b, "pct_diff": pct_diff(pnl_a, pnl_b)},
                    "fees": {"fallback": fees_a, "vectorbt": fees_b, "diff": None if fees_a is None or fees_b is None else fees_a - fees_b},
                    "mfe": {"fallback": mfe_a, "vectorbt": mfe_b, "diff": None if mfe_a is None or mfe_b is None else mfe_a - mfe_b},
                    "mae": {"fallback": mae_a, "vectorbt": mae_b, "diff": None if mae_a is None or mae_b is None else mae_a - mae_b},
                    "fallback": ft,
                    "vectorbt": vt,
                }
                matches.append(match)
            else:
                matches.append({"fallback_idx": i, "vectorbt_idx": None, "fallback": ft, "vectorbt": None})
    ...
    def main() -> int:
        ...
        cfg_obj = config.model_dump() if hasattr(config, "model_dump") else (config.dict() if hasattr(config, "dict") else {})
    ...
    for i, ft in enumerate(fb_trades):
        ...
        if (
            ...
            from dateutil import parser
            try:
                if ft.get("exit_time") and vt.get("exit_time"):
                    edt = (
                        parser.parse(ft["exit_time"]) - parser.parse(vt["exit_time"])
                    ).total_seconds()
                    exit_time_diff = abs(edt)
            except Exception:
                exit_time_diff = None
            ...
            match = {
                "fallback_idx": i,
                "vectorbt_idx": best_j,
                def pairwise_match(fb_trades, vb_trades, max_time_diff_seconds=86400):
                    """Match fallback trades to vectorbt trades by side + nearest entry_time.
                    ...
                    for t in fb_trades:
                        t["entry_dt"] = get_dt(t.get("entry_time"))
                    for t in vb_trades:
                        t["entry_dt"] = get_dt(t.get("entry_time"))
                    ...
                    for i, ft in enumerate(fb_trades):
                        ...
                        if best_j is not None and best_dt is not None and best_dt <= max_time_diff_seconds:
                            ...
                            exit_time_diff = None
                            try:
                                et_a = get_dt(ft.get("exit_time"))
                                et_b = get_dt(vt.get("exit_time"))
                                if et_a and et_b:
                                    exit_time_diff = abs((et_a - et_b).total_seconds())
                            except Exception:
                                exit_time_diff = None
