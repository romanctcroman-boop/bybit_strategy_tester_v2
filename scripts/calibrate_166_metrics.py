...
class TradingViewMetric:
    ...
    tv_value_all: Optional[float] = None
    tv_value_long: Optional[float] = None
    tv_value_short: Optional[float] = None
    calculated_value_all: Optional[float] = None
    calculated_value_long: Optional[float] = None
    calculated_value_short: Optional[float] = None
    tolerance_percent: float = 0.5  # Допустимое отклонение %
    tolerance_absolute: Optional[float] = None  # Абсолютное допустимое отклонение
...
def print_calibration_report(results: List[Dict]):
    """Вывести отчёт о калибровке."""
    print("\n" + "="*90)
    print("📊 ОТЧЁТ О КАЛИБРОВКЕ МЕТРИК - TradingView Golden Reference")
    print("="*90)
    ...
    print(f"   ❌ Расхождения: {mismatched}")
    print(f"   ⚠️ Отсутствуют: {missing}")
    
    print("\n" + "-"*90)
    print(f"{'Метрика':<30} {'Ожидаемое':>12} {'Факт':>12} {'Разница':>10} {'%':>8} {'Статус':<12}")
    print("-"*90)
    ...
    for r in results:
        ...
        print(f"{r['name_ru']:<30} {expected:>12} {actual:>12} {diff:>10} {diff_pct:>8} {r['status']:<12}")
