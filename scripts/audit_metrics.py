...
def run_audit():
    ...
    try:
        ...
        end_date = datetime(2026, 1, 17, tzinfo=UTC)
        ...
        print(
            f"total_trades: {m.total_trades} (actual: {len(trades)}) {'✅' if m.total_trades == len(trades) else '❌'}"
        )
        print(
            f"winning_trades: {m.winning_trades} (actual: {len(winning)}) {'✅' if m.winning_trades == len(winning) else '❌'}"
        )
        print(
            f"losing_trades: {m.losing_trades} (actual: {len(losing)}) {'✅' if m.losing_trades == len(losing) else '❌'}"
        )

        expected_win_rate = (len(winning) / len(trades) * 100) if trades else 0
        print(
            f"win_rate: {m.win_rate:.2f}% (expected: {expected_win_rate:.2f}%) {'✅' if abs(m.win_rate - expected_win_rate) < 0.1 else '❌'}"
        )
        ...
        print(
            f"gross_profit: {m.gross_profit:.2f} (actual: {actual_gross_profit:.2f}) {'✅' if abs(m.gross_profit - actual_gross_profit) < 1 else '❌'}"
        )
        print(
            f"gross_loss: {m.gross_loss:.2f} (actual: {actual_gross_loss:.2f}) {'✅' if abs(m.gross_loss - actual_gross_loss) < 1 else '❌'}"
        )

        expected_pf = actual_gross_profit / actual_gross_loss if actual_gross_loss > 0 else 0
        print(
            f"profit_factor: {m.profit_factor:.4f} (expected: {expected_pf:.4f}) {'✅' if abs(m.profit_factor - expected_pf) < 0.01 else '❌'}"
        )
        ...
        print(
            f"avg_win_value: {m.avg_win_value:.2f} (actual: {actual_avg_win:.2f}) {'✅' if abs(m.avg_win_value - actual_avg_win) < 1 else '❌'}"
        )
        print(
            f"avg_loss_value: {m.avg_loss_value:.2f} (actual: {actual_avg_loss:.2f}) {'✅' if abs(m.avg_loss_value - actual_avg_loss) < 1 else '❌'}"
        )
        print(
            f"avg_trade_value: {m.avg_trade_value:.2f} (actual: {actual_avg_trade:.2f}) {'✅' if abs(m.avg_trade_value - actual_avg_trade) < 1 else '❌'}"
        )
        ...
        print(
            f"long_trades: {m.long_trades} (actual: {len(long_trades)}) {'✅' if m.long_trades == len(long_trades) else '❌'}"
        )
        print(
            f"short_trades: {m.short_trades} (actual: {len(short_trades)}) {'✅' if m.short_trades == len(short_trades) else '❌'}"
        )
        ...
        print(
            f"long_winning_trades: {m.long_winning_trades} (actual: {len(long_winning)}) {'✅' if m.long_winning_trades == len(long_winning) else '❌'}"
        )
        print(
            f"short_winning_trades: {m.short_winning_trades} (actual: {len(short_winning)}) {'✅' if m.short_winning_trades == len(short_winning) else '❌'}"
        )
        ...
        print(
            f"avg_bars_in_trade: {m.avg_bars_in_trade:.2f} (actual: {actual_avg_bars:.2f}) {'✅' if abs(m.avg_bars_in_trade - actual_avg_bars) < 0.5 else '⚠️ check'}"
        )
        ...
        print("NOTE: These metrics are calculated from equity curve, not tick data")
        ...
        print(
            f"buy_hold_return_pct: {m.buy_hold_return_pct:.2f}% (expected: {expected_bh_pct:.2f}%) {'✅' if abs(m.buy_hold_return_pct - expected_bh_pct) < 1 else '⚠️ check'}"
        )
