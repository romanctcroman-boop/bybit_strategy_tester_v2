...
def main():
    ...
    for our_idx in range(min(35, len(our_trades))):
        ...
        if tv_match_idx is not None:
            ...
            print(
                f"{our_idx + 1:>4} | {str(our['entry_time'])[:19]:>19} | {our['direction']:>6} | ${our['entry_price']:>9.2f} || {tv_match_idx + 1:>4} | {str(tv['datetime'])[:19]:>19} | {tv['direction']:>6} | ${tv['price']:>9.2f} | {match}"
            )
        else:
            print(
                f"{our_idx + 1:>4} | {str(our['entry_time'])[:19]:>19} | {our['direction']:>6} | ${our['entry_price']:>9.2f} || {'N/A':>4} | {'':>19} | {'':>6} | {'':>10} | ❌ NO MATCH"
            )
    ...
    for bt in boundary_triggers:
        ...
        print(
            f"  Trade ~#{bt['trade_num']:>2} | {str(bt['datetime'])[:19]} | {bt['direction']:>5} | RSI: {bt['prev_rsi']:.4f} → {bt['curr_rsi']:.4f} | Δ={bt['distance']:.6f}{marker}"
        )
    ...
    for i, bt in enumerate(sorted_boundaries[:5]):
        print(
            f"  #{i + 1}: {bt['datetime']} | RSI={bt['curr_rsi']:.6f} | Δ from {bt['threshold']}={bt['distance']:.6f} | {bt['direction']}"
        )
