...
FROM bybit_kline_audit WHERE symbol='BTCUSDT' AND interval='60' ORDER BY open_time ASC LIMIT 500""",
    conn,
)
...
df_1m = pd.read_sql(
    """SELECT open_time, open_price as open, high_price as high, low_price as low, close_price as close
FROM bybit_kline_audit WHERE symbol='BTCUSDT' AND interval='1' ORDER BY open_time ASC""",
    conn,
)
