...
def load_tv_trades():
    ...
    for _, row in entries.iterrows():
        ...
        result.append({"direction": direction, "price": price, "datetime": str(dt_col)})
...
def main():
    ...
    for our_idx, our in our_df.iterrows():
        ...
        if best_match is not None and best_diff < 50:  # $50 tolerance
            ...
            print(
                f"{our_idx + 1:>4} | {str(our['entry_time'])[:19]:>19} | {our['direction']:>6} | ${our['entry_price']:>9.2f} || {best_match + 1:>4} | {tv['direction']:>6} | ${tv['price']:>9.2f} | {match_str}"
            )
        else:
            print(
                f"{our_idx + 1:>4} | {str(our['entry_time'])[:19]:>19} | {our['direction']:>6} | ${our['entry_price']:>9.2f} || {'N/A':>4} | {'':>6} | {'':>10} | ❌ NO MATCH"
            )
