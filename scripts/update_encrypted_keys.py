"""
Update encrypted_secrets.json with new API keys
"""
import sys
sys.path.insert(0, 'd:/bybit_strategy_tester_v2')

from backend.security.key_manager import get_key_manager, KeyManager
...
def main():
    ...
    # Reset singleton to pick up new .env values
    KeyManager.reset_instance()
    km = get_key_manager()
    
    # Collect all keys we want to encrypt
    keys_to_encrypt = {
        # Perplexity
    ...
    print(f"\n📦 Encrypting {len(keys_to_encrypt)} keys...")
    
    try:
        km.save_encrypted_keys(keys_to_encrypt)
    ...
    for key_name in ["PERPLEXITY_API_KEY", "DEEPSEEK_API_KEY"]:
        try:
            value = km.get_decrypted_key(key_name)
            print(f"  ✅ {key_name}: {value[:15]}...")
        except Exception as e:
            print(f"  ❌ {key_name}: {e}")
