...
engine = get_engine(strategy_type="dca", pyramiding=PYRAMIDING, max_entries=PYRAMIDING)
print(f"\nEngine: {engine.name}")
...
if result_long.trades:
    ...
    for i, t in enumerate(result_long.trades[:5], 1):
        print(
            f"    #{i}: Entry ${t.entry_price:.0f} → Exit ${t.exit_price:.0f}, PnL ${t.pnl:.2f} ({t.pnl_pct * 100:.2f}%)"
        )
...
print(f"{'-' * 60}")
print(
    f"{'DCA LONG':<15} {len(result_long.trades):<10} ${result_long.metrics.net_profit:<14.2f} {result_long.metrics.total_return:<9.2f}% {result_long.metrics.win_rate * 100:<9.1f}%"
)
print(
    f"{'DCA SHORT':<15} {len(result_short.trades):<10} ${result_short.metrics.net_profit:<14.2f} {result_short.metrics.total_return:<9.2f}% {result_short.metrics.win_rate * 100:<9.1f}%"
)
