...
print(
    f"   {'Trades':20s}: FB={len(fb_result.trades):12d}  GPU={len(gpu_result.trades):12d}  {'OK' if trades_match else 'FAIL'}"
)
...
for i in range(min(5, len(fb_result.trades))):
    ...
    print(
        f"   Trade {i + 1}: {fb_t.direction:5s} PnL=${fb_t.pnl:8.2f} vs ${gpu_t.pnl:8.2f}  Exit={fb_t.exit_reason.name} vs {gpu_t.exit_reason.name}  {status}"
    )
