...
for i in range(min(len(result_fallback.trades), n_trades)):
    ...
    print(f"\n{status} Trade {i + 1}:")
    print(
        f"   Type:      FB={('LONG' if fb_is_long else 'SHORT'):5} | NUMBA={('LONG' if numba_is_long else 'SHORT'):5}"
    )
...
print("\n" + "=" * 80)
print(
    f"TOTAL PnL: FB={total_pnl_fb:.2f} | NUMBA={total_pnl_numba:.2f} | diff={abs(total_pnl_fb - total_pnl_numba):.2f}"
)
print("=" * 80)
