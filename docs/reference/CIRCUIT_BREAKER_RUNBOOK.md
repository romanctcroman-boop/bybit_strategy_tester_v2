...
| `backend/api/perplexity_client.py` | ✅ Complete | Integrated with `AgentCircuitBreakerManager`. Uses `call_with_breaker()` for health checks. Breaker name: `perplexity_api`. |
| `backend/api/deepseek_client.py` | ✅ Complete | Integrated with `AgentCircuitBreakerManager`. Uses `call_with_breaker()` for health checks. Breaker name: `deepseek_api`. |
| `backend/mcp/mcp_integration.py` | ✅ Complete | Integrated with `AgentCircuitBreakerManager`. Uses `_execute_with_breaker()` with progressive timeouts (60→120→300→600s). Breaker name: `mcp_server`. |
...
📌 **Action Plan (Updated December 12, 2025):**

1. ~~Add breaker-aware wrappers to the standalone DeepSeek/Perplexity health clients~~ ✅ **DONE**
2. ~~Inject `AgentCircuitBreakerManager` into the MCP bridge~~ ✅ **DONE**
