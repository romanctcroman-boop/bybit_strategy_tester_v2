...
def _make_ohlcv(
    n: int = 200, trend: float = 0.0, volatility: float = 0.015, start_price: float = 85000.0, seed: int = 42
) -> pd.DataFrame:
    """
...
class TestAgentPersonas:
    ...
    def test_agents_diverge_on_same_market(self):
        ...
        sizes = [ds.position_management.size_pct, qw.position_management.size_pct, per.position_management.size_pct]
...
class TestDeliberationDynamics:
    ...
    def test_deliberation_round_structure(self):
        ...
        print("  ✓ Структура DeliberationResult валидна")
    ...
    def test_deliberation_convergence_increases_confidence(self):
        ...
        print("  ✓ Конвергенция зафиксирована")
    ...
    def test_deliberation_disagreement_uses_more_rounds(self):
        ...
        assert len(result.rounds) >= 2, f"При конфликте должно быть ≥2 раундов, получено {len(result.rounds)}"
    ...
    def test_cross_examination_references_other_agents(self):
        ...
        if len(received_prompts) > 2:
            ...
            if has_peer_reference:
                print("  ✓ Cross-examination промпт содержит ссылки на позиции других агентов")
            else:
                print("  ⚠ Cross-examination промпты не ссылаются явно на другие позиции (возможно норма)")

        print(f"  Всего промптов получено: {len(received_prompts)}")
        print("  ✓ Перекрёстный допрос завершён")
...
class TestConsensusFormation:
    ...
    def test_dynamic_weights_from_performance_history(self):
        ...
        assert weights["deepseek"] > weights["qwen"], (
            f"DeepSeek (лучший Sharpe) должен иметь больший вес: DS={weights['deepseek']:.3f}, QW={weights['qwen']:.3f}"
        )
    ...
    def test_consensus_engine_aggregates_strategies(self):
        ...
        print("  ✓ Консенсус построен корректно")
    ...
    def test_agreement_score_higher_for_similar_strategies(self):
        ...
        print("  ✓ Agreement scores в допустимом диапазоне [0,1]")
...
class TestMemoryLifecycle:
    ...
    def test_store_and_recall_episodic(self):
        ...
        print("  ✓ Эпизодическая память работает")
    ...
    def test_working_memory_is_stored(self):
        ...
        print("  ✓ WORKING tier доступен немедленно")
    ...
    def test_semantic_memory_high_importance_persists(self):
        ...
        print("  ✓ Долгосрочная память сохранена")
    ...
    def test_importance_filtering_excludes_low_quality(self):
        ...
        assert len(high_results) <= len(all_results), "Фильтр по важности не должен добавлять записи"
        ...
        print("  ✓ Фильтрация по важности работает")
    ...
    def test_cross_agent_shared_namespace(self):
        ...
        print("  ✓ Cross-agent namespace работает")
...
class TestFullPipelineSanity:
    ...
    def test_pipeline_executes_all_nodes(self, ohlcv_bullish):
        ...
        from backend.agents.langgraph_orchestrator import AgentState
        from backend.agents.trading_strategy_graph import GenerateStrategiesNode, build_trading_strategy_graph
        ...
        async def fake_generate(self_node, state: AgentState) -> AgentState:
            ...
            state.set_result(
                "generate_strategies", {"responses": [{"agent": "deepseek", "response": _DEEPSEEK_STRATEGY_JSON}]}
            )
        ...
        print("  ✓ Все ключевые ноды выполнены")
    ...
    def test_analyze_market_detects_bullish_regime(self, ohlcv_bullish):
        ...
        print("  ✓ MarketContext построен для бычьих данных")
    ...
    def test_analyze_market_detects_bearish_regime(self, ohlcv_bearish):
        ...
        print(f"  Тренд                 : {result.get('trend', '?')}")
        print("  ✓ MarketContext для медвежьих данных")
    ...
    def test_debate_node_enriches_state(self, ohlcv_bullish):
        ...
        print("  ✓ DebateNode обогатил state.context")
...
class TestCostCircuitBreakerUnderPressure:
    ...
    def test_normal_calls_pass_through(self):
        ...
        print("  ✓ Нормальные вызовы не блокируются")
    ...
    def test_hourly_limit_blocks_after_accumulation(self):
        ...
        print("  ✓ Часовой лимит накопления работает")
    ...
    def test_circuit_breaker_rolling_window_expires(self):
        ...
        print("  ✓ Rolling window корректно очищается")
    ...
    def test_circuit_breaker_integrates_with_agent_interface(self):
        ...
        mock_breaker.check_before_call.side_effect = CostLimitExceededError("Blocked", "per_call", 0.001, 0.50)

        with patch("backend.agents.cost_circuit_breaker.get_cost_circuit_breaker", return_value=mock_breaker):
            from backend.agents.unified_agent_interface import AgentRequest, AgentType, UnifiedAgentInterface
        ...
        print("  ✓ Circuit Breaker интегрирован в UnifiedAgentInterface")
...
class TestLLMCacheDeduplication:
    ...
    def test_identical_prompts_produce_same_key(self):
        ...
        print(f"\n  Cache key: {key1}")
        print("  ✓ Идентичные промпты → одинаковый ключ")
    ...
    def test_timestamp_normalization_produces_cache_hit(self):
        ...
        assert key1 == key2 == key3, (
            f"Промпты с разными timestamp должны давать одинаковый ключ. Получено: {key1}, {key2}, {key3}"
        )
        print("  ✓ Временные метки нормализованы: все три промпта → один ключ")
    ...
    def test_dollar_price_normalization(self):
        ...
        print("  ✓ Цены нормализованы: разные BTC цены → один ключ")
    ...
    def test_set_then_get_returns_response(self):
        ...
        assert cached.get("content") == original_response["content"], f"Содержимое кэша должно совпадать: {cached}"

        print(f"\n  Кэшировано : '{original_response['content'][:50]}...'")
        print(f"  Из кэша    : '{cached.get('content', '')[:50]}...'")
        print("  ✓ set() → get() работает корректно")
    ...
    def test_different_prompts_miss_cache(self):
        ...
        print("  ✓ Разные промпты → разные ключи (cache miss гарантирован)")
...
class TestStressScenarios:
    ...
    def test_consensus_node_with_single_proposal(self):
        ...
        state = AgentState()
        state.set_result(
            "parse_responses", {"proposals": [{"agent": "deepseek", "strategy": strategy, "validation": validation}]}
        )
        ...
        print("  ✓ Graceful degradation при 1 кандидате")
    ...
    def test_parse_node_handles_malformed_json(self):
        ...
        print("  ✓ Graceful degradation при невалидных ответах")
    ...
    def test_empty_ohlcv_causes_graceful_error(self):
        ...
        print("  ✓ Пустые данные обработаны грациозно")
    ...
    def test_all_agents_produce_conflicting_strategies(self):
        ...
        print(f"  Выбрал агент    : {result['selected_agent']}")
        print(f"  Agreement score : {result['agreement_score']:.2%}")
        print("  ✓ Консенсус справился с тотальным конфликтом")
    ...
    def test_memory_node_skips_when_no_backtest(self):
        ...
        assert len(final_state.errors) == 0, "MemoryUpdateNode не должна генерировать ошибки при пустом backtest"
        ...
        print("  ✓ Корректный пропуск без backtest результатов")
