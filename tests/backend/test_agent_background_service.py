...
def create_api_key(
    ...
    key = APIKey(value=value, agent_type=agent_type, index=index)
    ...
    async def test_init_logs_success(self):
        ...
        with patch("backend.agents.agent_background_service.get_agent_interface"):
            with patch("backend.agents.agent_background_service.logger") as mock_logger:
                _ = AIAgentBackgroundService()

                # Logger.success may be called multiple times during init
                # (for Unified Agent Interface and FallbackService)
                assert mock_logger.success.call_count >= 1
                # Check that at least one call mentions initialization
                all_calls = str(mock_logger.success.call_args_list)
                assert "initialized" in all_calls.lower()
...
class TestMCPServerChecks:
    ...
    async def test_check_mcp_server_uses_breaker_when_available(self):
        ...
        with patch(
            ...
            circuit_manager.call_with_breaker = AsyncMock(
