...
from backend.mcp.mcp_integration import MCPFastAPIBridge, get_mcp_bridge
...
class TestMCPBridgePerToolBreakersIntegration:
    ...
    async def test_bridge_initialization_registers_breakers(self, real_bridge):
        ...
        assert len(real_bridge.circuit_breakers) > 0, (
            "Per-tool breakers should be registered after initialization"
        )
        
        # Check that categories are assigned
        assert len(real_bridge.breaker_categories) > 0, (
            "Tool categories should be assigned after initialization"
        )
        
        # Check that metrics are initialized
        assert len(real_bridge.tool_metrics) > 0, (
            "Tool metrics should be initialized after initialization"
        )
    ...
    async def test_breaker_categories_assigned(self, real_bridge):
        ...
        for tool_name in real_bridge._tools.keys():
            assert tool_name in real_bridge.breaker_categories, (
                f"Tool '{tool_name}' should have a category"
            )
            
            category = real_bridge.breaker_categories[tool_name]
            assert category in ["high", "medium", "low"], (
                f"Category for '{tool_name}' should be high/medium/low"
            )
        ...
        for tool_name in high_tools:
            if tool_name in real_bridge._tools:
                ...
                assert category == "high", (
                    f"Tool '{tool_name}' should be high criticality"
                )
    ...
    async def test_metrics_initialized(self, real_bridge):
        ...
        for tool_name in real_bridge._tools.keys():
            assert tool_name in real_bridge.tool_metrics, (
                f"Tool '{tool_name}' should have metrics"
            )
            ...
            required_fields = [
                ...
                "circuit_breaks", "last_call", "last_error", "avg_latency_ms"
            ]
            for field in required_fields:
                assert field in metrics, (
                    f"Metrics for '{tool_name}' should have '{field}' field"
                )
    ...
    async def test_isolated_breaker_behavior(self, real_bridge):
        ...
        assert breaker1 != breaker2, (
            f"Tools '{tool1}' and '{tool2}' should have different breakers"
        )
    ...
    async def test_category_based_thresholds(self, real_bridge):
        ...
        categories_with_tools = [
            cat for cat, tools in by_category.items() if tools
        ]
        
        assert len(categories_with_tools) >= 2, (
            "Should have tools in at least 2 categories"
        )
...
class TestMCPBridgeMetricsRecording:
    ...
    async def test_metrics_recorded_on_call(self, initialized_bridge):
        ...
        assert current_calls > initial_calls, (
            "Calls counter should increment on tool call"
        )
...
class TestCircuitBreakerPerformance:
    ...
    async def test_breaker_overhead(self, initialized_bridge):
        ...
        assert avg_per_call < 10, (
            f"Circuit breaker overhead too high: {avg_per_call:.2f}ms per call"
        )
