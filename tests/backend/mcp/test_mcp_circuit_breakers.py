...
class TestMCPFastAPIBridgeCircuitBreaker:
    ...
    async def test_circuit_breaker_open_behavior(self, bridge):
        ...
        bridge._tools = {
            "test_tool": MagicMock(
                name="test_tool",
                callable=AsyncMock(side_effect=Exception("Tool failed"))
            )
        }
    ...
    async def test_progressive_retry_on_timeout(self, bridge):
        ...
        bridge._tools = {
            "slow_tool": MagicMock(name="slow_tool", callable=mock_tool_call)
        }
    ...
    async def test_no_retry_on_non_timeout_error(self, bridge):
        ...
        bridge._tools = {
            "bad_tool": MagicMock(name="bad_tool", callable=mock_tool_call)
        }
...
class TestCircuitBreakerPerformance:
    ...
    async def test_circuit_breaker_overhead(self):
        ...
        # Overhead should be < 10ms per call (100 calls = < 1000ms)
        assert elapsed < 1.0, f"Circuit breaker overhead too high: {elapsed}s for 100 calls"
