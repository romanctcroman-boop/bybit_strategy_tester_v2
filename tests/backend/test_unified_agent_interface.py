...
class TestAgentRequest:
    ...
    def test_agent_request_to_direct_api_format_deepseek(self):
        """AgentRequest converts to DeepSeek API format"""
        request = AgentRequest(
            agent_type=AgentType.DEEPSEEK,
            task_type="fix",
...
class TestAPIKeyManager:
    ...
    @patch('backend.security.key_manager.KeyManager')
    def test_get_active_key_deepseek(self, mock_km_class):
        ...
        key = manager.get_active_key(AgentType.DEEPSEEK)
        
        assert key is not None
        assert key.value == "key2"  # Lower error count
    
    @patch('backend.security.key_manager.KeyManager')
    def test_get_active_key_perplexity(self, mock_km_class):
        ...
        key = manager.get_active_key(AgentType.PERPLEXITY)
        
        assert key is not None
        assert key.value == "pk1"
    
    @patch('backend.security.key_manager.KeyManager')
    def test_get_active_key_none_available(self, mock_km_class):
    ...
    @patch('backend.security.key_manager.KeyManager')
    def test_get_active_key_prefers_fewer_errors(self, mock_km_class):
    ...
    @patch('backend.security.key_manager.KeyManager')
    def test_mark_error_disables_after_three(self, mock_km_class):
        """mark_error disables key after 3 errors"""
        mock_km = MagicMock()
        mock_km_class.return_value = mock_km
        mock_km.get_decrypted_key.return_value = "test"
...
class TestUnifiedAgentInterfaceRequests:
    ...
    async def test_send_request_increments_total(self, mock_km_class):
        ...
        mock_km = MagicMock()
        mock_km_class.return_value = mock_km
        mock_km.get_active_key.return_value = None  # No keys, will fail
    ...
    async def test_send_request_falls_back_to_direct_api(self, mock_km_class):
        ...
        with patch.object(interface, '_try_mcp', new_callable=AsyncMock) as mock_mcp:
            with patch.object(interface, '_try_direct_api', new_callable=AsyncMock) as mock_direct:
    ...
    async def test_send_request_handles_mcp_exception(self, mock_km_class):
        ...
        with patch.object(interface, '_try_mcp', side_effect=Exception("MCP crash")):
            with patch.object(interface, '_try_direct_api', return_value=AgentResponse(
                success=True, content="Fallback success", channel=AgentChannel.DIRECT_API
            )):
    ...
    @pytest.mark.asyncio
    @patch('backend.security.key_manager.KeyManager')
    async def test_try_mcp_returns_not_implemented(self, mock_km_class):
        """_try_mcp returns not implemented error"""
        mock_km = MagicMock()
        mock_km_class.return_value = mock_km
        mock_km.get_decrypted_key.return_value = "test_key"
...
class TestConvenienceFunctions:
    ...
    async def test_analyze_with_deepseek_creates_request(self, mock_get_interface):
        ...
        response = await analyze_with_deepseek("def test(): pass", focus="bugs")
    ...
    async def test_ask_perplexity_creates_request(self, mock_get_interface):
        ...
        response = await ask_perplexity("What is Bitcoin?")
...
class TestIntegration:
    ...
    async def test_full_workflow_deepseek_analysis(self, mock_client_class, mock_km_class):
        ...
        mock_km.get_active_key.return_value = key
