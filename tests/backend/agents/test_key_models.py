...
class TestAPIKeyInit:
    ...
    def test_init_with_value_kwarg(self):
        ...
        key = APIKey(value="sk-test-123", agent_type=AgentType.CLAUDE, index=0)
        assert key.value == "sk-test-123"
        assert key.agent_type == AgentType.CLAUDE
        assert key.index == 0

    def test_init_with_value_override(self):
        """Initialize with value_override= (backward compat)."""
        key = APIKey(agent_type=AgentType.CLAUDE, index=1, value_override="sk-claude-456")
        assert key.value == "sk-claude-456"
    ...
    def test_init_auto_generates_key_name(self):
        ...
        key = APIKey(agent_type=AgentType.CLAUDE, index=0)
        assert key.key_name == "CL-1"

        key2 = APIKey(agent_type=AgentType.COPILOT, index=2)
        assert key2.key_name == "CP-3"
    ...
    def test_init_with_explicit_key_name(self):
        ...
        key = APIKey(value="sk-test", agent_type=AgentType.CLAUDE, index=0, key_name="MY-KEY")
