...
def mock_candle_cache():
    """Mock CANDLE_CACHE service in both marketdata.py and marketdata_cache.py."""
    with (
        patch("backend.api.routers.marketdata.CANDLE_CACHE") as mock1,
        patch("backend.api.routers.marketdata_cache.CANDLE_CACHE") as mock2,
    ):
        # Link both mocks to have the same behavior
        mock2.load_initial = mock1.load_initial
        mock2.get_working_set = mock1.get_working_set
        mock2.reset = mock1.reset
        mock2.LOAD_LIMIT = mock1.LOAD_LIMIT
        yield mock1
...
class TestFetchMtf:
    ...
    def test_fetch_mtf_aligned_success(self, mock_mtf_manager):
        ...
        assert set(data["data"].keys()) == {"1", "15", "60"}
        mock_mtf_manager.get_aligned.assert_called_once_with(
            "BTCUSDT", ["1", "15", "60"], base_interval=None, load_limit=1000
        )

    def test_fetch_mtf_preserves_interval_payload(self, mock_mtf_manager):
        """Ensure response contains candle payloads per interval."""
        mock_result = MagicMock()
        mock_result.symbol = "BTCUSDT"
        mock_result.intervals = ["1", "60"]
        mock_result.data = {
            "1": [
                {
                    "time": 1700000000,
        ...
        assert r.status_code == 200
        body = r.json()
        assert body["intervals"] == ["1", "60"]
        assert body["data"]["1"][0]["time"] == 1700000000
        assert body["data"]["60"][0]["close"] == 115.0
        assert {c["close"] for c in body["data"]["1"]} == {108.0}
