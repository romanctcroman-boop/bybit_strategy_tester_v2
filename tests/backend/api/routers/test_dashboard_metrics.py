...
from backend.models import Backtest, Strategy, Optimization, Trade


client = TestClient(app)


def create_mock_backtest(id, strategy_id, symbol, status, sharpe_ratio=None, 
                         total_return=None, win_rate=None, max_drawdown=None, 
                         total_trades=0, created_at=None, started_at=None, 
                         completed_at=None, strategy_name="Test Strategy"):
...
class TestMetricsSummary:
    ...
    def test_get_metrics_summary_24h(self, mock_session_local, mock_db_session, sample_backtests, sample_strategies, sample_optimizations):
        ...
        mock_backtest_query.count.side_effect = [3, 1, 1, 1]  # total, completed, running, failed
...
class TestStrategyMetrics:
    ...
    @pytest.mark.skip(reason="Requires database - run with: pytest -m integration")
    @patch('backend.api.routers.dashboard_metrics.SessionLocal')
    def test_get_strategy_metrics_success(self, mock_session_local, mock_db_session, sample_strategies, sample_backtests):
...
class TestSystemHealth:
    ...
    @patch('backend.api.routers.dashboard_metrics.SessionLocal')
    def test_get_system_health_success(self, mock_session_local, mock_db_session):
    ...
    def test_get_system_health_database_error(self, mock_get_db):
