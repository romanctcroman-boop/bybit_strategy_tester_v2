...
def _print_strategy(strategy, source: str = "") -> None:
    ...
    if strategy.position_management:
        print(
            f"    position    : {strategy.position_management.size_pct}% x{strategy.position_management.max_positions}"
        )
...
async def test_qwen_live_strategy_generation():
    ...
    if strategy:
        ...
        print("  ✅ QWEN живой и техничен!")
    else:
        print("  ⚠️  QWEN ответил, парсер не распознал формат.")
...
async def test_perplexity_live_strategy_generation():
    ...
    if strategy:
        ...
        print("  ⚠️  Perplexity ответил, парсер не распознал формат.")
...
async def test_self_moa_temperature_diversity_live():
    ...
    print("  Запускаем 3 параллельных вызова T=[0.3, 0.7, 1.1]...")
    ...
    if len(texts) >= 2:
        ...
        if len(unique) == 1:
            print("  ⚠️  Все температуры дали ОДИНАКОВЫЙ текст — детерминизм?")
        else:
            ...
            print("  ✅ Self-MoA реально даёт разнообразие!")
...
async def test_mad_debate_deepseek_vs_qwen_live():
    ...
    print(f"\n  Вопрос: {question[:120]}...")
    print("  Агенты: DeepSeek (квант) vs QWEN (технарь)")
    print("  Максимум раундов: 2")
...
async def test_memory_pipeline_store_and_recall_live():
    ...
    from backend.agents.prompts.response_parser import Signal, StrategyDefinition
    ...
    if len(results) == 2:
        ...
        if ds_s and qw_s:
            ...
            if ds_s.optimization_hints and qw_s.optimization_hints:
                print(
                    f"  Цель оптимизации: DeepSeek={ds_s.optimization_hints.primary_objective}, "
