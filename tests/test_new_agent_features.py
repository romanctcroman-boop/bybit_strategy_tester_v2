...
async def test_mcp():
    ...
    try:
        from backend.agents.mcp.protocol import (
            MCPServer, MCPClient, InMemoryTransport, MCPMessage, MCPResource, MCPPrompt
        )
...
async def test_tool_registry():
    ...
    try:
        from backend.agents.mcp.tool_registry import (
            ToolRegistry, Tool, ToolParameter, ParameterType
        )
...
async def test_resource_manager():
    ...
    try:
        from backend.agents.mcp.resource_manager import (
            ResourceManager, MemoryResourceProvider
        )
...
async def test_context_manager():
    ...
    try:
        from backend.agents.mcp.context_manager import (
            ContextManager, Context, ContextScope
        )
...
async def test_llm_connections():
    ...
    try:
        from backend.agents.llm.connections import (
            LLMProvider, LLMMessage, LLMResponse, LLMConfig,
            RateLimiter, LLMClientPool
        )
...
async def test_shared_memory():
    ...
    try:
        from backend.agents.memory.shared_memory import (
            SharedMemory, ConflictResolution, Transaction
        )
...
async def test_ml_anomaly():
    ...
    try:
        ...
        from backend.agents.monitoring.ml_anomaly import (
            ZScoreDetector, IQRDetector, MovingAverageDetector,
            EnsembleDetector, MLAnomalyDetector, AnomalySeverity
        )
...
async def test_communication():
    ...
    try:
        from backend.agents.communication.protocol import (
            MessageBroker, Message, MessageType, MessagePriority,
            AgentInfo, AgentCommunicator
        )
...
async def test_prometheus_grafana():
    ...
    try:
        from backend.agents.monitoring.prometheus_grafana import (
            PrometheusConfig, GrafanaConfig, PrometheusExporter,
            GrafanaDashboard, create_ai_agent_dashboard
        )
...
async def test_dashboard_api():
    ...
    try:
        from backend.agents.dashboard.api import (
            MetricQuery, AlertCreate, DashboardWidget
        )
