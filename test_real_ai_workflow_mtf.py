"""
🚀 РАСШИРЕННЫЙ РЕАЛЬНЫЙ AI WORKFLOW С MULTI-TIMEFRAME ТЕСТИРОВАНИЕМ

Улучшения:
1. ✅ Загрузка ПОЛНЫХ 3 месяцев данных (не 1000 свечей)
2. ✅ Multi-Timeframe: 5m, 15m, 30m одновременно
3. ✅ Множественные циклы оптимизации
4. ✅ Grid Search по параметрам от Perplexity
5. ✅ Walk-Forward валидация
6. ✅ Реальная работа с данными через backfill
...
class MTFAIWorkflowOrchestrator:
    """Расширенный оркестратор с Multi-Timeframe поддержкой."""
    
    def __init__(self):
        self.dialogue_history = []
        self.test_results = {}
        self.current_date = datetime(2025, 10, 29)
        self.test_period_start = self.current_date - timedelta(days=90)  # 3 месяца
        self.timeframes = ['5', '15', '30']  # MTF: 5m, 15m, 30m
        self.central_tf = '15'  # Центральный таймфрейм
    ...
    async def phase1_mtf_strategy_design(self):
        """ФАЗА 1: Дизайн Multi-Timeframe стратегии через Perplexity."""
        print("\n" + "="*80)
        print("📋 ФАЗА 1: ДИЗАЙН MTF СТРАТЕГИИ ЧЕРЕЗ PERPLEXITY")
        print("="*80)
        ...
        if result["success"]:
            print(f"\n🔮 Perplexity MTF Strategy Design:")
    ...
    async def phase2_data_requirements(self):
        """ФАЗА 2: Расчёт требований к данным через Perplexity."""
        print("\n" + "="*80)
        print("📊 ФАЗА 2: РАСЧЁТ ТРЕБОВАНИЙ К ДАННЫМ")
        print("="*80)
    ...
    async def phase3_load_full_mtf_data(self, symbol: str):
        """ФАЗА 3: Загрузка ПОЛНЫХ 3 месяцев данных для всех таймфреймов."""
        print("\n" + "="*80)
        print("📥 ФАЗА 3: ЗАГРУЗКА ПОЛНЫХ MTF ДАННЫХ (3 МЕСЯЦА)")
        print("="*80)
        ...
        try:
            ...
            with get_session() as session:
                ...
                for tf in self.timeframes:
                    print(f"\n📊 Загрузка {tf}m данных для {symbol}...")
                    print(f"   Период: {self.test_period_start.strftime('%Y-%m-%d')} → {self.current_date.strftime('%Y-%m-%d')}")
                    
                    # Используем backfill для загрузки
                    ...
                    print(f"   ✅ Загружено: {result['upserts']} свечей")
                    print(f"   API запросов: {result['pages']}")
                    ...
                    if not df.empty:
                        ...
                        print(f"   ⚠️ Нет данных в БД для {tf}m")
    ...
    async def phase3_fallback_load_data(self, symbol: str):
        ...
        for tf in self.timeframes:
            print(f"\n📊 Загрузка {tf}m (fallback - макс 1000 свечей)...")
    ...
    async def phase4_grid_optimization(self, mtf_data: Dict[str, pd.DataFrame], parameter_matrix: List[Dict]):
        """ФАЗА 4: Grid Search оптимизация по параметрам от Perplexity."""
        print("\n" + "="*80)
        print("⚙️ ФАЗА 4: GRID SEARCH ОПТИМИЗАЦИЯ")
        print("="*80)
        
        from backend.core.backtest_engine import BacktestEngine
        
        # Используем данные центрального таймфрейма для простого бэктеста
        central_data = mtf_data.get(self.central_tf)
        
        if central_data is None or central_data.empty:
            print("❌ Нет данных для центрального таймфрейма")
            return None
        
        print(f"\n🎯 Тестируем {len(parameter_matrix)} комбинаций параметров...")
        print(f"   Данные: {len(central_data)} свечей на {self.central_tf}m")
        
        optimization_results = []
        
        for i, params in enumerate(parameter_matrix, 1):
            print(f"\n   🔄 Вариант {i}/{len(parameter_matrix)}: {params}")
        ...
        if valid_results:
            ...
            print(f"\n🏆 ТОП-3 ЛУЧШИХ КОМБИНАЦИЙ (по Sharpe):")
    ...
    async def phase5_ai_analysis_optimization(self, optimization_results: List[Dict]):
        """ФАЗА 5: AI анализ результатов оптимизации."""
        print("\n" + "="*80)
        print("🔬 ФАЗА 5: AI АНАЛИЗ РЕЗУЛЬТАТОВ ОПТИМИЗАЦИИ")
        print("="*80)
        
        # Формируем summary для Perplexity
        valid_results = [r for r in optimization_results if 'error' not in r]
    ...
    async def run_full_mtf_workflow(self, symbol: str = "BTCUSDT"):
        """Запуск полного MTF workflow."""
        print("\n" + "🌟"*40)
        print("🚀 РАСШИРЕННЫЙ AI WORKFLOW: MTF + ОПТИМИЗАЦИЯ")
        print("🌟"*40)
        print(f"\nСимвол: {symbol}")
    ...
    async def generate_final_report(self):
        ...
        print(f"\n✅ MTF отчёт сохранён: {report_path}")
        print(f"\n📊 СТАТИСТИКА:")
        print(f"   Обороты Copilot ↔ Perplexity: {len(self.dialogue_history)}")
        print(f"   Таймфреймы протестированы: {len(self.timeframes)}")
        
        if "mtf_data_loaded" in self.test_results:
            print(f"\n📥 ЗАГРУЖЕННЫЕ ДАННЫЕ:")
            for tf, info in self.test_results["mtf_data_loaded"].items():
                print(f"   {tf}m: {info['candles']} свечей")
        
        if "grid_optimization" in self.test_results:
            valid = len([r for r in self.test_results["grid_optimization"] if 'error' not in r])
            print(f"\n⚙️ ОПТИМИЗАЦИЯ:")
            print(f"   Протестировано комбинаций: {valid}")
...
if __name__ == "__main__":
    print("\n" + "🌟"*40)
    print("ЗАПУСК РАСШИРЕННОГО MTF AI WORKFLOW")
