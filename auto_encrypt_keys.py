...
def _collect_keys(prefix: str, max_keys: int = 16) -> dict[str, str]:
    """Return all environment variables matching prefix and prefix_N."""
    collected: dict[str, str] = {}

    base_value = os.getenv(prefix)
    if base_value:
        collected[prefix] = base_value
...
def main():
    ...
    deepseek_keys = _collect_keys("DEEPSEEK_API_KEY")
    perplexity_keys = _collect_keys("PERPLEXITY_API_KEY")

    if not deepseek_keys and not perplexity_keys:
    ...
    if deepseek_keys:
        for name, value in deepseek_keys.items():
            secrets[name] = value
            print(f"✅ {name} найден: {value[:10]}...")
    
    if perplexity_keys:
        for name, value in perplexity_keys.items():
            secrets[name] = value
            print(f"✅ {name} найден: {value[:10]}...")
