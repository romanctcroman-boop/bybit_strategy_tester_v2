"""
Run Walk-Forward Optimization for S/R Mean-Reversion Strategy
This is an adapted lightweight runner that mirrors run_wfo_full.py structure but uses SRMeanReversionStrategy.
"""
import json
import logging
...
def backtest_sr_strategy(data: pd.DataFrame, params: dict, initial_capital: float = 10000.0, commission: float = 0.00055) -> dict:
    """Full backtest with PnL, Sharpe, win rate, max drawdown"""
    strat = SRMeanReversionStrategy(**params)
...
def run_wfo():
    ...
    results = []
    param_space = [
        {'lookback_bars': 80, 'level_tolerance_pct': 0.08, 'entry_tolerance_pct': 0.10, 'stop_loss_pct': 0.6, 'max_holding_bars': 36},
        {'lookback_bars': 100, 'level_tolerance_pct': 0.10, 'entry_tolerance_pct': 0.15, 'stop_loss_pct': 0.8, 'max_holding_bars': 48},
        {'lookback_bars': 120, 'level_tolerance_pct': 0.12, 'entry_tolerance_pct': 0.20, 'stop_loss_pct': 1.0, 'max_holding_bars': 60},
    ]
    ...
    while (start_idx + in_sample + out_sample) <= total_bars and period < 22:
        ...
        for p in param_space:
            # placeholder: score = number of detected levels (proxy)
            detector = SRMeanReversionStrategy(**p)
            detector.on_start(is_df)
            levels = detector.sr_detector.detect_levels(is_df)
            score = len(levels.get('support', [])) + len(levels.get('resistance', []))
            if score > best_score:
                best_score = score
                best = p
        logger.info(f' Best params: {best} (score {best_score})')
        oos_result = backtest_sr_strategy(oos_df, best)
        oos_metrics = {
    ...
    out_path = f'results/wfo_sr_22_cycles_{now}.json'
