...
def test_klines():
    ...
    try:
        ...
        response = requests.get(
            f"{BASE_URL}/api/v1/marketdata/bybit/klines/fetch",
            params=params,
            timeout=30
        )
        print(f"    Status: {response.status_code}")
        ...
        print("\n[4] Data sample:")
        print(f"    First candle (oldest):")
        print(f"      Time: {first['open_time']} ({datetime.fromtimestamp(first['open_time']/1000)})")
        print(f"      OHLCV: {first['open']}, {first['high']}, {first['low']}, {first['close']}, {first['volume']}")
        
        print(f"\n    Last candle (newest):")
        print(f"      Time: {last['open_time']} ({datetime.fromtimestamp(last['open_time']/1000)})")
        print(f"      OHLCV: {last['open']}, {last['high']}, {last['low']}, {last['close']}, {last['volume']}")
        ...
        print(f"    Expected for 1-min candles: 60 seconds")
        
        if abs(avg_diff / 1000 - 60) > 5:
            print(f"    WARNING: Time differences don't match 1-min interval")
        else:
            print(f"    ✓ Time intervals are correct for 1-min candles")
        
        # Check data ordering
        print("\n[6] Data ordering:")
        if times[0] < times[-1]:
            print(f"    ✓ Data is newest-first (timestamp increases)")
        else:
            print(f"    ✗ Data ordering issue")
