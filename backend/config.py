...
def _build_config():
    if SETTINGS is None:
        ...
        return _ns(
            ...
            ws_symbols=[
                s.strip().upper()
                for s in os.environ.get("BYBIT_WS_SYMBOLS", "BTCUSDT").split(",")
                if s.strip()
            ],
            ws_intervals=[
                s.strip().upper()
                for s in os.environ.get("BYBIT_WS_INTERVALS", "1").split(",")
                if s.strip()
            ],
        )
    ...
    redis_url = (
        getattr(getattr(SETTINGS, "redis", _ns(url="redis://127.0.0.1:6379/0")), "url", None)
