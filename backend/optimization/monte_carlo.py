...
class MonteCarloSimulator:
    ...
    def _calculate_return(self, trades: List[Dict[str, Any]], initial_capital: float) -> float:
        """
        Расчёт доходности в % с компаундингом
        
        ВАЖНО: Используем compounding, чтобы возврат зависел от порядка сделок.
        Это критично для Monte Carlo - разные последовательности дают разные результаты.
        ...
        for trade in trades:
            pnl = trade.get('pnl', 0)
            # Compounding: процент прибыли зависит от текущего капитала
