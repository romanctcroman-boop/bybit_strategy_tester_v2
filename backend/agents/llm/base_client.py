            ...
            try:
                async with session.post(
                    ...
                    if resp.status == 429:
                        ...
                        logger.warning(
                            f"{self.EMOJI} Rate limited, waiting {retry_after}s (attempt {attempt + 1}/{self.config.max_retries})"
                        )
                        if attempt < self.config.max_retries - 1:
                            await asyncio.sleep(retry_after)
                            continue
                        raise RuntimeError(
                            f"{type(self).__name__}: rate limit exceeded after {self.config.max_retries} attempts"
                        )
