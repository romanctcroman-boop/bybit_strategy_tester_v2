...
class HealthMonitor:
    ...
    def __init__(
        ...
        if self._mcp_disabled:
            logger.warning("🚫 MCP health monitoring set to decommissioned mode via MCP_DISABLED flag")
    ...
    def _build_decommissioned_result(self, context: Optional[str] = None) -> HealthCheckResult:
    ...
    def _update_fallback_state(self, status: HealthStatus, reason: str, context: Optional[str] = None) -> None:
    ...
    def _start_degraded_period(self, reason: str, context: Optional[str] = None) -> None:
        entry = {
    ...
    async def probe_mcp_server(self, context: Optional[str] = None) -> HealthCheckResult:
        if self._mcp_disabled:
    ...
    async def _maybe_restart_mcp(self) -> None:
        ...
        if self._last_restart_attempt and (now - self._last_restart_attempt).total_seconds() < CONTROLLED_RESTART_COOLDOWN_SECONDS:
            return
