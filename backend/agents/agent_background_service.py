...
from backend.agents.circuit_breaker_manager import CircuitState, CircuitBreakerError
...
class AIAgentBackgroundService:
    ...
    async def _test_deepseek_connection(self):
        ...
        try:
            ...
            if active_keys:
                ...
                logger.success(f"✅ DeepSeek: Ready ({len(active_keys)}/{len(km.deepseek_keys)} keys usable)")
            else:
    ...
    async def _test_perplexity_connection(self):
        ...
        try:
            ...
            if active_keys:
                # Check if at least one key is available (no actual API call)
                logger.success(f"✅ Perplexity: Ready ({len(active_keys)}/{len(km.perplexity_keys)} keys usable)")
            else:
    ...
    def _print_health_summary(self):
        ...
        logger.info(f"🚦 MCP breaker skips: {self.stats['mcp_breaker_rejections']}")
        logger.info("─" * 80)
