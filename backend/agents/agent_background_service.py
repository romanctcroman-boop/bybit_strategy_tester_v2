...
import sys
import time
from datetime import datetime, timezone  # timezone for UTC timestamps
...
class AIAgentBackgroundService:
    ...
    async def _comprehensive_health_check(self):
        ...
        is_full_check = (current_time - self.last_full_check) >= self.full_health_check_interval
    ...
    async def _check_api_keys(self):
        ...
        deepseek_active = sum(1 for k in km.deepseek_keys if k.is_active)
        deepseek_total = len(km.deepseek_keys)
        
        perplexity_active = sum(1 for k in km.perplexity_keys if k.is_active)
        perplexity_total = len(km.perplexity_keys)
        
        logger.info(f"🔑 DeepSeek keys: {deepseek_active}/{deepseek_total} active")
        logger.info(f"🔑 Perplexity keys: {perplexity_active}/{perplexity_total} active")
        
        # Check if need to rotate keys (all have errors)
        if deepseek_active == 0 and deepseek_total > 0:
            logger.warning("⚠️ All DeepSeek keys disabled, resetting error counts...")
        ...
        if perplexity_active == 0 and perplexity_total > 0:
            logger.warning("⚠️ All Perplexity keys disabled, resetting error counts...")
    ...
    async def _check_mcp_server(self):
        ...
        try:
            async with httpx.AsyncClient(timeout=self.mcp_health_timeout) as client:
                # Prefer native FastAPI health endpoint which exposes tool registry
                ...
                is_available = all(tool in tools for tool in required_tools) and data.get("status") in ("healthy", "degraded")

                if is_available:
                    logger.debug(f"✅ MCP tools registered: {len(tools)} total, agent tools available")
                else:
        ...
        if previous != is_available:
            ...
            logger.info(
                f"🔄 MCP availability changed → {'Available' if is_available else 'Unavailable'}"
            )
        
        status_icon = "✅" if is_available else "❌"
        logger.info(f"{status_icon} MCP Server: {'Available' if is_available else 'Unavailable'}")
    
    async def _test_deepseek_connection(self):
        """Легковесная проверка DeepSeek (без реальных API calls)"""
        try:
            km = self.interface.key_manager
            active_keys = [k for k in km.deepseek_keys if k.is_active]
            
            if active_keys:
                # Check if at least one key is available (no actual API call)
                logger.success(f"✅ DeepSeek: Ready ({len(active_keys)}/{len(km.deepseek_keys)} keys active)")
            else:
                logger.warning("⚠️ DeepSeek: No active keys available")
    ...
    async def _test_perplexity_connection(self):
        ...
        try:
            km = self.interface.key_manager
            active_keys = [k for k in km.perplexity_keys if k.is_active]
            
            if active_keys:
                # Check if at least one key is available (no actual API call)
                logger.success(f"✅ Perplexity: Ready ({len(active_keys)}/{len(km.perplexity_keys)} keys active)")
            else:
                logger.warning("⚠️ Perplexity: No active keys available")
    ...
    async def _test_deepseek_connection_full(self):
        ...
        try:
            ...
            if response.success:
                logger.success(f"✅ DeepSeek: Connected (key #{response.api_key_index}, {response.latency_ms:.0f}ms)")
            else:
    ...
    async def _test_perplexity_connection_full(self):
        ...
        try:
            ...
            if response.success:
                logger.success(f"✅ Perplexity: Connected (key #{response.api_key_index}, {response.latency_ms:.0f}ms)")
            else:
    ...
    def _print_health_summary(self):
        ...
        logger.info(f"✅ MCP success: {stats['mcp_success']}")
        logger.info(f"❌ MCP failed: {stats['mcp_failed']}")
        logger.info(f"✅ Direct API success: {stats['direct_api_success']}")
        logger.info(f"❌ Direct API failed: {stats['direct_api_failed']}")
        logger.info(f"🔄 Key rotations: {self.stats['api_key_rotations']}")
        logger.info("─" * 80)
