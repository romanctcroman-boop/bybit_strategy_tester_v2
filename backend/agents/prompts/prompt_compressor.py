...
class PromptCompressor:
    ...
    def __init__(
        ...
        logger.info(f"✂️ PromptCompressor initialized (max_tokens={max_tokens}, target={target_compression:.0%})")
    ...
    def compress(
        ...
        logger.info(f"✂️ Compressed: {current_tokens} → {new_tokens} tokens (saved {saved}, {ratio:.0%})")
    ...
    def _aggressive_compression(
        ...
        for line in lines:
            ...
            if any(re.search(p, line, flags=re.IGNORECASE) for p in self.KEY_PATTERNS) or len(line) < 50:
