...
class TemperatureAdapter:
    ...
    def _adjust_for_confidence(self, confidence: float) -> float:
        ...
        if confidence >= self.config.high_confidence_threshold:
            ...
            range_size = self.config.high_confidence_threshold - self.config.low_confidence_threshold
            position = (confidence - self.config.low_confidence_threshold) / range_size

            # Interpolate between multipliers
            multiplier_range = self.config.low_confidence_multiplier - self.config.high_confidence_multiplier
    ...
    def get_temperature_for_agent(
        ...
        # Clamp
        return max(self.config.min_temperature, min(temp, self.config.max_temperature))
