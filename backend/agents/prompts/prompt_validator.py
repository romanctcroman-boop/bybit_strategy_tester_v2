...
class PromptValidator:
    ...
    def __init__(
        ...
        logger.info(
            f"🛡️ PromptValidator initialized "
            f"(max_len={self.max_length}, injections={block_injections})"
        )
    ...
    def _check_length(self, prompt: str, result: ValidationResult) -> None:
        ...
        if length < self.min_length:
            result.errors.append(
                f"Prompt too short: {length} chars (min: {self.min_length})"
            )
            result.is_valid = False
        
        if length > self.max_length:
            result.errors.append(
                f"Prompt too long: {length} chars (max: {self.max_length})"
            )
            result.is_valid = False
        ...
        # Warning for long prompts
            ...
            result.warnings.append(
                f"Prompt approaching length limit: {length}/{self.max_length}"
            )
    ...
    def _check_injections(self, prompt: str, result: ValidationResult) -> None:
        ...
        for i, pattern in enumerate(self._injection_regex):
            ...
            if match:
                ...
                result.errors.append(
                    f"Injection attempt detected: '{pattern_str}'"
                )
                result.is_valid = False
                logger.warning(
                    f"🚫 Prompt injection blocked: {pattern_str}"
                )
    ...
    def _check_sensitive_data(self, prompt: str, result: ValidationResult) -> None:
        ...
        for i, pattern in enumerate(self._sensitive_regex):
            ...
            if match:
                ...
                result.errors.append(
                    f"Sensitive data detected: '{pattern_str}'"
                )
                result.is_valid = False
                logger.warning(
                    f"🚫 Sensitive data blocked: {pattern_str}"
                )
    ...
    def _check_structure(self, prompt: str, result: ValidationResult) -> None:
        ...
        # Check for only whitespace
            ...
            result.errors.append(
                f"Prompt contains only whitespace ({len(prompt)} chars)"
            )
            result.is_valid = False
