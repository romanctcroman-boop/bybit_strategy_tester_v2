...
class SHAPExplainer:
    ...
    def explain(self, features: pd.DataFrame, feature_names: list[str] | None = None) -> SignalExplanation:
        """
        ...
        signal = self.model.predict(features)[0] if hasattr(self.model, "predict") else 0.5
    ...
    def _calculate_importance(self, features: pd.DataFrame, feature_names: list[str]) -> dict[str, float]:
        """
        Calculate feature importance (simplified SHAP).
    ...
    def _generate_recommendation(self, signal: float, top_features: list[tuple[str, float]]) -> str:
...
class LIMEExplainer:
    ...
    def explain_instance(self, instance: pd.Series, n_samples: int = 1000) -> SignalExplanation:
        """
        ...
        _weights, coefficients = self._fit_local_model(samples, predictions, instance)
    ...
    def _perturb(self, instance: pd.Series, n_samples: int) -> np.ndarray:
        """Generate perturbed samples"""
        samples = []
    ...
    def _fit_local_model(
        self, samples: np.ndarray, predictions: np.ndarray, instance: pd.Series
    ...
    def _get_top_features(self, coefficients: np.ndarray, feature_names: list[str], n: int = 5) -> list[dict[str, Any]]:
        """Get top N features"""
