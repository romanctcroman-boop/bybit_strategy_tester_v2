...
@cached(ttl=120, key_prefix="strategies:list")  # Cache for 2 minutes
def list_strategies(
...
@cached(ttl=300, key_prefix="strategy:detail")  # Cache for 5 minutes
...
@invalidate_cache(patterns=["strategies:list:*"])  # Invalidate list cache
...
@invalidate_cache(patterns=["strategies:list:*", "strategy:detail:*"])  # Invalidate on update
...
@invalidate_cache(patterns=["strategies:list:*", "strategy:detail:*"])  # Invalidate on delete
