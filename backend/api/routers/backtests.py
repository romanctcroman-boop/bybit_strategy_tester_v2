...
@cached(ttl=60, key_prefix="backtests:list")  # Cache for 60 seconds
def list_backtests(
...
@cached(ttl=300, key_prefix="backtest:detail")  # Cache for 5 minutes
...
@invalidate_cache(patterns=["backtests:list:*"])  # Invalidate list cache on create
...
@invalidate_cache(patterns=["backtests:list:*", "backtest:detail:*"])  # Invalidate on update
