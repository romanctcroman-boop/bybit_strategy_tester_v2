...
Provides aggregated stats for backtests, strategies, optimizations, and system health.
"""
...
async def get_metrics_summary(
    ...
    try:
        ...
        completed_backtests = backtest_query.filter(Backtest.status == 'completed').count()
        running_backtests = backtest_query.filter(Backtest.status == 'running').count()
        failed_backtests = backtest_query.filter(Backtest.status == 'failed').count()
        ...
        active_strategies = strategy_query.filter(Strategy.is_active == True).count()
        
        # Optimization metrics
        opt_query = db.query(Optimization)
        if time_filter:
            opt_query = opt_query.filter(Optimization.created_at >= time_filter)
        
        total_optimizations = opt_query.count()
        running_optimizations = opt_query.filter(Optimization.status == 'running').count()
        completed_optimizations = opt_query.filter(Optimization.status == 'completed').count()
        
        # Performance metrics
        avg_duration = db.query(
            func.avg(
                func.extract('epoch', Backtest.completed_at - Backtest.started_at)
            )
        ).filter(
            and_(
                Backtest.status == 'completed',
                Backtest.completed_at.isnot(None),
                Backtest.started_at.isnot(None)
            )
        ).scalar()
        
        # Total trades analyzed
        trade_query = db.query(func.count(Trade.id))
        if time_filter:
            trade_query = trade_query.join(Backtest).filter(Backtest.created_at >= time_filter)
        total_trades = trade_query.scalar() or 0
        ...
        return {
            ...
            "performance": {
                "avg_backtest_duration_sec": round(avg_duration or 0, 2),
...
async def get_top_performers(
    ...
    metric: str = Query("sharpe_ratio", description="Metric to rank by: sharpe_ratio, total_return, win_rate")
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        query = db.query(Backtest).join(Strategy).filter(
            and_(
                Backtest.status == 'completed',
...
async def get_system_health() -> dict[str, Any]:
    ...
    try:
        ...
        pending_backtests = db.query(Backtest).filter(
