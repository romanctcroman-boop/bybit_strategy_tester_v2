...
async def check_celery_health() -> ComponentHealth:
    ...
    try:
        ...
        return ComponentHealth(
            ...
            status=status,
            response_time_ms=round(response_time, 2),
            details={"worker_count": worker_count, "active_tasks": total_tasks, "workers": list(active_workers.keys())},
            last_check=utc_now().isoformat().replace("+00:00","Z")
        )
    except Exception as e:
        response_time = (time.time() - start) * 1000
...
async def check_disk_health() -> ComponentHealth:
    ...
    try:
        ...
        return ComponentHealth(
            ...
            status=status,
            response_time_ms=round(response_time, 2),
            details={"used_pct": round(used_pct, 2), "free_gb": round(free_gb, 2), "total_gb": round(total_gb, 2), "path": current_dir},
            last_check=utc_now().isoformat().replace("+00:00","Z")
        )
    except Exception as e:
        response_time = (time.time() - start) * 1000
...
async def check_api_health() -> ComponentHealth:
    """Check external API (Bybit) connectivity"""
    start = time.time()
    try:
        ...
        response_time = (time.time() - start) * 1000
        status = "healthy" if len(candles) > 0 else "degraded"
        return ComponentHealth(
            name="bybit_api",
            status=status,
            response_time_ms=round(response_time, 2),
            details={"candles_fetched": len(candles), "latest_price": float(candles[-1]['close']) if candles else None},
            last_check=utc_now().isoformat().replace("+00:00","Z")
        )
    except Exception as e:
        response_time = (time.time() - start) * 1000
...
async def health_dashboard():
    ...
    summary = {"healthy": statuses.count("healthy"), "degraded": statuses.count("degraded"), "unhealthy": statuses.count("unhealthy")}
...
async def get_component_health(component_name: str):
    ...
    if component_name not in component_checks:
        raise HTTPException(status_code=404, detail=f"Component '{component_name}' not found. Available: {list(component_checks.keys())}")
