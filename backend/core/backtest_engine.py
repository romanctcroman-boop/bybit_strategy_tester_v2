...
- ML-оптимизация параметров через CatBoost/XGBoost/LightGBM
"""
...
class BacktestEngine:
    ...
    def run(self, data: pd.DataFrame, strategy_config: dict) -> dict[str, Any]:
        ...
        CRITICAL: Comprehensive error handling added
        """
        try:
            # Input validation
            if data is None:
            ...
            try:
                ...
                logger.error(f"Backtest loop failed at bar {i}: {e}")
                # Try to salvage results up to the failure point
                logger.info(f"Returning partial results up to bar {i}/{len(df)}")
    ...
    def _check_entry(
        ...
        - Use only data up to current bar (i and i-1)
        - Never access future bars (i+1, i+2, etc.)
        """
        ...
        if strategy_type == 'ema_crossover':
            ...
            # ✅ FIXED: Only use current (i) and past (i-1) data
            if pd.isna(ema_fast.iloc[i]) or pd.isna(ema_slow.iloc[i]):
                return
            if i > 0 and (pd.isna(ema_fast.iloc[i - 1]) or pd.isna(ema_slow.iloc[i - 1])):
                return
            
            # Long signal: fast crosses above slow
            # ✅ SAFE: Uses only i and i-1 (no look-ahead)
    ...
    async def ml_optimize(
        ...
        """
        ML-оптимизация параметров стратегии через CatBoost/XGBoost/LightGBM
        ...
        Args:
            ...
            optimization_goal: Целевая метрика ('sharpe_ratio', 'win_rate', 'total_return', 'profit_factor')
            ml_library: ML-библиотека ('catboost', 'xgboost', 'lightgbm', 'hybrid')
            method: Метод поиска ('grid', 'random', 'bayes')
            n_trials: Количество итераций (для random/bayes)
        ...
        try:
            ...
            logger.error(f"ML module not available: {e}")
            raise ImportError(
                "ML optimizers not found. Install: pip install catboost xgboost lightgbm optuna"
            )
        
        logger.info(f"Starting ML-optimization with {ml_library} ({method} search)")
        logger.info(f"Parameter space: {param_space}")
        logger.info(f"Optimization goal: {optimization_goal}")
        
        # Создать objective function для оптимизатора
        def objective_function(params: Dict[str, Any]) -> float:
            """
            Запустить бэктест с параметрами и вернуть целевую метрику
            
            Returns:
                Значение целевой метрики (выше = лучше)
            """
            try:
                # Создать конфигурацию стратегии из параметров
                strategy_config = {
                    'type': 'ml_optimized',
                    **params
                }
                ...
                # Запустить бэктест
                results = self.run(data, strategy_config)
                
                # Извлечь целевую метрику
                if optimization_goal == 'sharpe_ratio':
                    score = results.get('sharpe_ratio', 0.0)
                elif optimization_goal == 'win_rate':
                    score = results.get('win_rate', 0.0)
                elif optimization_goal == 'total_return':
                    score = results.get('total_return', 0.0)
                elif optimization_goal == 'profit_factor':
                    score = results.get('profit_factor', 0.0)
                elif optimization_goal == 'sortino_ratio':
                    score = results.get('sortino_ratio', 0.0)
                else:
                    # Попробовать найти метрику в metrics dict
                    score = results.get('metrics', {}).get(optimization_goal, 0.0)
                
                # Штраф за малое количество сделок (нужна статистическая значимость)
        ...
        if ml_library == 'catboost':
            ...
            raise ValueError(f"Unknown ML library: {ml_library}")
        
        # Запустить оптимизацию
        result = await optimizer.optimize(n_trials=n_trials, method=method)
        
        # Запустить финальный бэктест с лучшими параметрами
        best_strategy_config = {'type': 'ml_optimized', **result.best_params}
        final_results = self.run(data, best_strategy_config)
        
        # Добавить метрики финального бэктеста в результат
        result.sharpe_ratio = final_results.get('sharpe_ratio', 0.0)
        result.max_drawdown = final_results.get('max_drawdown', 0.0)
        result.win_rate = final_results.get('win_rate', 0.0)
        result.profit_factor = final_results.get('profit_factor', 0.0)
        result.total_return = final_results.get('total_return', 0.0)
        
        logger.info(f"✓ ML-optimization completed: {result.method}, {result.iterations} iterations")
        logger.info(f"  Best {optimization_goal}: {result.best_score:.4f}")
    ...
    async def auto_optimize(
        self,
        data: pd.DataFrame,
        strategy_type: str = 'sr_rsi',
        optimization_goal: str = 'sharpe_ratio',
        quick_mode: bool = False
    ) -> Dict[str, Any]:
        """
        Автоматическая оптимизация с предустановленными параметрами
        
        Args:
            data: Исторические данные
            strategy_type: Тип стратегии ('sr_rsi', 'ema_crossover', 'scalping')
            optimization_goal: Целевая метрика
            quick_mode: Быстрый режим (меньше итераций)
        
        Returns:
            Результат оптимизации
        """
        logger.info(f"Auto-optimize strategy: {strategy_type}, goal: {optimization_goal}")
        
        # Предустановленные пространства параметров для разных стратегий
        if strategy_type == 'sr_rsi':
            param_space = {
                'sr_lookback': [20, 50, 100, 150, 200] if not quick_mode else [50, 100],
                'sr_threshold': [0.001, 0.002, 0.005, 0.01] if not quick_mode else [0.002, 0.005],
                'rsi_period': [7, 14, 21, 28] if not quick_mode else [14, 21],
                'rsi_overbought': [65, 70, 75, 80] if not quick_mode else [70, 75],
                'rsi_oversold': [20, 25, 30, 35] if not quick_mode else [25, 30],
                'take_profit_pct': [0.01, 0.02, 0.03, 0.05] if not quick_mode else [0.02, 0.03],
                'stop_loss_pct': [0.005, 0.01, 0.015, 0.02] if not quick_mode else [0.01, 0.015],
            }
        elif strategy_type == 'ema_crossover':
            param_space = {
                'fast_period': [5, 10, 15, 20] if not quick_mode else [10, 15],
                'slow_period': [20, 50, 100, 200] if not quick_mode else [50, 100],
                'take_profit_pct': [0.02, 0.03, 0.05] if not quick_mode else [0.02, 0.03],
                'stop_loss_pct': [0.01, 0.015, 0.02] if not quick_mode else [0.01, 0.015],
            }
        elif strategy_type == 'scalping':
            param_space = {
                'entry_threshold': [0.001, 0.002, 0.003] if not quick_mode else [0.001, 0.002],
                'take_profit_pct': [0.003, 0.005, 0.01] if not quick_mode else [0.005, 0.01],
                'stop_loss_pct': [0.002, 0.003, 0.005] if not quick_mode else [0.002, 0.003],
                'rsi_period': [7, 14, 21] if not quick_mode else [14],
            }
        else:
            raise ValueError(f"Unknown strategy type: {strategy_type}")
        
        # Выбрать ML-библиотеку и метод
        if quick_mode:
            ml_library = 'lightgbm'  # Самый быстрый
            method = 'random'
            n_trials = 30
        else:
            ml_library = 'hybrid'  # Самый точный
            method = 'bayes'
            n_trials = 100
        
        # Запустить оптимизацию
        result = await self.ml_optimize(
            data=data,
            param_space=param_space,
