...
from backend.models import (
    Strategy, Backtest, Trade, Optimization, 
    OptimizationResult, MarketData
)
...
class DataService:
    ...
    def claim_backtest_to_run(self, backtest_id: int, now: datetime, stale_seconds: int = 24*3600) -> ClaimResult:
        ...
        if status == "running" and started_at:
            ...
            if delta < stale_seconds:
                return ClaimResult(status="running", backtest=bt, message="Already running and not stale")
    ...
    def get_trades_count(self, backtest_id: int) -> int:
        ...
        return self.db.query(func.count(Trade.id)).filter(
    ...
    def get_best_optimization_result(
        ...
        return self.db.query(OptimizationResult).filter(
            OptimizationResult.optimization_id == optimization_id
        ).order_by(desc(OptimizationResult.score)).first()
