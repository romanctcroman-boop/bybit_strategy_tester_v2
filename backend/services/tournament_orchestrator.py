...
class TournamentOrchestrator:
    ...
    def _build_drift_snapshot(
        ...
        models_needing_retrain = (
            self.drift_monitor.get_models_needing_retrain(config.drift_retrain_threshold)
    ...
    async def _handle_drift_tracking(
        ...
        self._performance_baseline[strategy.strategy_id] = (
            (1 - alpha) * baseline + alpha * actual_return
        )
    ...
    async def _trigger_emergency_retrain(
        ...
        try:
            ...
            logger.info(
                "✅ Emergency retrain complete for %s", strategy.strategy_name
            )
