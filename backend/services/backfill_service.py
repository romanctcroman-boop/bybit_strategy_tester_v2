...
class BackfillService:
    ...
    def backfill(self, cfg: BackfillConfig, *, resume: bool = True, return_stats: bool = False):
        ...
        while pages < cfg.max_pages:
            ...
            inserted = self._persist(cfg.symbol, cfg.interval, rows)
            done += inserted
    ...
    def _persist(self, symbol: str, interval: str, rows: list[dict]) -> int:
        # Use BybitAdapter persistence helper (idempotent upsert), decoupled from fetch adapter
        try:
            BybitAdapter()._persist_klines_to_db(symbol, rows, interval=interval)
            return len(rows)
        except Exception as e:
