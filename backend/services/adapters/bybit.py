...
>>>>>>> Stashed changes
        ...
        if isinstance(row, list):
            ...
            start_candidates = [raw.get('startTime'), raw.get('start_at'), raw.get('t'), raw.get('open_time'), raw.get('startTime')]
            start_ms = None
            for cand in start_candidates:
                ...
                try:
                    ...
                    try:
                        ...
                        continue
            parsed['open_time'] = start_ms
            parsed['open_time_dt'] = datetime.fromtimestamp(start_ms / 1000.0, tz=timezone.utc) if start_ms is not None else None
    ...
    def _persist_klines_to_db(self, symbol: str, normalized_rows: List[Dict], db: Optional[object] = None, engine: Optional[object] = None):
        ...
        for row in normalized_rows:
            ...
            params.append({
                'symbol': symbol,
                'open_time': open_time,
                'open_time_dt': row.get('open_time_dt') if row.get('open_time_dt') is not None else _to_dt(open_time),
        ...
        if _engine is None:
            ...
            _engine = _create_engine(_database_url, connect_args={"check_same_thread": False} if _database_url.startswith('sqlite') else {})
        ...
        def _is_session_like(o):
            return o is not None and (hasattr(o, 'execute') or hasattr(o, 'query') or hasattr(o, 'begin') or hasattr(o, 'get_bind'))
        ...
        try:
            if dialect in ('postgres', 'postgresql'):
                ...
                for p in params:
                    existing = db.query(BybitKlineAudit).filter(BybitKlineAudit.symbol == p['symbol'], BybitKlineAudit.open_time == p['open_time']).one_or_none()
                    if existing:
    ...
    def _refresh_instruments_cache(self, force: bool = False):
        now = time.time()
        if not force and self._instruments_cache and self._instruments_cache_at and (now - self._instruments_cache_at) < self._instruments_cache_ttl:
            return
        v5_url_info = "https://api.bybit.com/v5/market/instruments-info"
        try:
            r = self._requests_get(
