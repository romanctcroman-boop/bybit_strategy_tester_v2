...
class RiskParityAllocator:
    ...
    def allocate(
        ...
        # Ограничения
        bounds = Bounds(
            lb=[self.min_weight] * n_assets,
            ub=[self.max_weight] * n_assets
        )
    ...
    def efficient_frontier(
        ...
        for target_vol in np.linspace(min_vol, max_vol, n_points):
            ...
            constraints = [
                ...
                {'type': 'ineq', 'fun': lambda x: target_vol - np.sqrt(x @ cov_matrix @ x)}
            ]
            ...
            if result.success:
                ...
                # Fallback: equal weights clipped to bounds
