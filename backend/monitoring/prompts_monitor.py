...
class PromptsMonitor:
    ...
    def get_dashboard(self, period_hours: int = 24) -> DashboardMetrics:
        ...
        if (
            ...
            and self._cached_metrics.period_hours == period_hours
            and (now - self._last_refresh) < self.config.refresh_interval_sec
        ):
    ...
    def get_validation_stats(self, period_hours: int = 24) -> dict[str, Any]:
        ...
        try:
            ...
            injection_attempts = sum(1 for log in logs if log.error_message and "Injection" in log.error_message)
