...
class AgentMetricsCollector:
    ...
    async def record_metric(self, metric: AgentMetric) -> None:
        ...
        try:
            ...
            logger.error(f"❌ Failed to record metric to Redis: {type(e).__name__}: {e}", exc_info=True)
    ...
    async def get_performance_summary(
        ...
        if success_metrics:
            ...
            performance.success_rate = performance.successful_requests / performance.total_requests if performance.total_requests > 0 else 0.0
        
        # Get tool calling metrics
        tool_metrics = await self._get_metrics_in_period(
            agent_name,
            MetricType.TOOL_CALLING,
            start_time,
            end_time
        )
