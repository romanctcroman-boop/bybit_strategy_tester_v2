...
logger = logging.getLogger("backend.security.key_manager")
...
class KeyManager:
    ...
    def _load_crypto_manager(self):
        ...
        try:
            ...
            logger.warning("Failed to initialize CryptoManager: %s", e)
            logger.info("API keys will only be available from environment variables")
    ...
    def _load_encrypted_secrets(self):
        ...
        if not encrypted_path.exists():
            # This is normal - env vars are preferred over encrypted file
            return
    ...
    def save_encrypted_keys(self, keys: dict[str, str]):
        ...
        logger.info("Saved %s encrypted keys to %s", len(keys), encrypted_path)
