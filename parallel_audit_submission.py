...
async def parallel_deepseek_audit(server: SimplifiedReliableMCP, audit_text: str):
    """Send 8 parallel requests to DeepSeek using 8 different API keys"""
    print(f"\n{'=' * 70}")
    print(f"🟣 DEEPSEEK PARALLEL AUDIT (8 concurrent requests)")
    print(f"{'=' * 70}")
    print(f"Original size: {len(audit_text):,} chars")
    
    # Split into 8 sections for 8 API keys
    sections = split_into_sections(audit_text, num_sections=8)
    prompts = create_parallel_prompts(sections, "DeepSeek", total=8)
    
    print(f"Split into: 8 sections")
    for i, (section, prompt) in enumerate(zip(sections, prompts), 1):
        print(f"  Part {i}: {len(section):,} chars → prompt {len(prompt):,} chars")
    
    # Create 8 parallel tasks (each uses different API key automatically via rotation)
    print(f"\n🚀 Launching 8 parallel requests...")
    tasks = [
    ...
    for i, result in enumerate(results, 1):
        if isinstance(result, Exception):
            print(f"  Part {i}/8: ❌ Error: {result}")
        elif "error" in result:
            print(f"  Part {i}/8: ❌ Failed: {result.get('error', 'Unknown')}")
        else:
            try:
                content = result.get("choices", [{}])[0].get("message", {}).get("content", "")
                print(f"  Part {i}/8: ✅ Success ({len(content):,} chars)")
                success_count += 1
            except:
                print(f"  Part {i}/8: ⚠️ Received but parsing failed")
    
    print(f"\n📊 Success Rate: {success_count}/8 ({success_count/8*100:.0f}%)")
    
    return results

async def parallel_perplexity_audit(server: SimplifiedReliableMCP, audit_text: str):
    """Send 4 parallel requests to Perplexity using 4 different API keys"""
    print(f"\n{'=' * 70}")
    print(f"🔵 PERPLEXITY PARALLEL AUDIT (4 concurrent requests)")
    print(f"{'=' * 70}")
    print(f"Original size: {len(audit_text):,} chars")
    
    # Split into 4 sections for 4 API keys
    sections = split_into_sections(audit_text, num_sections=4)
    prompts = create_parallel_prompts(sections, "Perplexity", total=4)
    
    print(f"Split into: 4 sections")
    for i, (section, prompt) in enumerate(zip(sections, prompts), 1):
        print(f"  Part {i}: {len(section):,} chars → prompt {len(prompt):,} chars")
    
    # Create 4 parallel tasks
    print(f"\n🚀 Launching 4 parallel requests...")
    tasks = [
    ...
    for i, result in enumerate(results, 1):
        if isinstance(result, Exception):
            print(f"  Part {i}/4: ❌ Error: {result}")
        elif "error" in result:
            print(f"  Part {i}/4: ❌ Failed: {result.get('error', 'Unknown')}")
        else:
            try:
                content = result.get("choices", [{}])[0].get("message", {}).get("content", "")
                print(f"  Part {i}/4: ✅ Success ({len(content):,} chars)")
                success_count += 1
            except:
                print(f"  Part {i}/4: ⚠️ Received but parsing failed")
...
async def main():
    ...
    deepseek_task = parallel_deepseek_audit(server, deepseek_request)
    perplexity_task = parallel_perplexity_audit(server, perplexity_request)
    ...
    save_parallel_results("DeepSeek", deepseek_results, num_parts=8)
    save_parallel_results("Perplexity", perplexity_results, num_parts=4)
    ...
    print(f"   DeepSeek: 8 parallel requests")
    print(f"   Perplexity: 4 parallel requests")
    print(f"   Total API calls: 12 concurrent")
    print(f"\n📁 Output Files:")
